...
function GateBadge({ blocked, label }) {
  return (
    <span
      data-testid={`gate-badge-${label.toLowerCase()}`}
    ...
    >
      {label} GATE — {blocked ? "BLOCKED" : "CLEAR"}
    </span>
  );
}
  ...
  return (
    <div className="mt-4 border border-[var(--border)] bg-[var(--surface-muted)] p-4"
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="section-tag">Final-job surface</div>
        <div className="flex gap-2">
          <GateBadge blocked={q.blocked} label="QUOTE" />
          <GateBadge blocked={o.blocked} label="ORDER" />
        </div>
      </div>
      ...
      {q.blocked && (
        <div className="mb-3" data-testid="quote-gate-blockers">
          <div className="text-xs font-bold text-red-700 mb-1">
            Quote gate — blocks email · Accept page · PDF · freeze · QR:
          </div>
      ...
      {(o.items || []).length > 0 && (
        <div data-testid="order-gate-items">
          <div className="text-xs font-bold mb-1">
            Order gate — taped fields close here and supersede derived values:
          </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        {gates.order_released ? (
          <div className="text-xs text-emerald-700" data-testid="order-released-stamp">
            Materials released by {gates.order_released.by} @ {gates.order_released.at}
          </div>
        ) : (
          <Button size="sm" disabled={o.blocked || releasing}
            data-testid="release-materials-btn" onClick={release}>
            {o.blocked ? "Order gate blocked" : releasing ? "Releasing…" : "Release materials"}
          </Button>
        )}
      </div>
    </div>
