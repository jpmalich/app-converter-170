...
- **DELIVERED**: Real-Job Delta Report for EST-562488 "3 Degree Rd" (`/app/memory/delta_report_3degree_rd_2026-07.md`) — ground truth xlsx vs app package; 11/11 printed quantities reproduced bit-exact from stored run `hover-dfd4847fcd71-board_batten`; every delta classified (a) formula/convention, (b) MISSING scope, (c) un-entered input, (d) Hover limitation. Headliners: 540-4" trim −110 sticks (frieze + B&B base scope missing), battens 212→465 (spacing ~8" o.c. suspected + pending height term), soffit 16→260 pcs (Hover's measured soffit_sqft 2620 dropped by passthrough — 2620/10 = 262 ≈ real 260 exact). Ruling questions Q9–Q17 queued.
- **DELIVERED (2026-07-27, THE RULING SITTING)**: Howard ruled Q1–Q17 in one pass; ALL SEVENTEEN LANDED AS CODE + tests (suite 1444 passed / 0 failed; new pin file `backend/tests/test_rulings_2026_07_27.py`). Highlights: battens 8" o.c. default (Q9), frieze + measured-soffit-total consumed — measured data never drops (Q10/Q14), ISC default re-SKU 540-4" merged with wrap (Q12), per-corner stick math on Hover (Q13), dormer SKUs retired → pooled (Q4), Tear-Off/Dumpster presence rows + readiness items (Q1), porch-implied flag (Q2), width-conditional fascia coil (Q3), photo mezzo pairing (Q5), stories/vents wired (Q6/Q7), per-estimate color-tier selector (Q8, `SettingsRow.jsx` + `color_tier` field), caulk/touch-up scaling (Q15), per-segment rounding (Q16), B&B waste holds 30% (Q17, evidence #1 logged), LP B&B base = nothing pinned (Q11). DERIVATION PURITY pinned by test. Named deltas: Jon Casile walk sub_mat 20,025.74→26,468.61 (full before→after in test pins); Haugh 540 33→62, OSC 9→20; Letrick 12,901.52→13,037.21. Acceptance: 3 Degree Rd re-derived — soffit/trim misses collapse to −42/−42 residuals tied to open flags. See `/app/memory/rulings_q1_q17_landed_2026-07-27.md`.
- **DELIVERED (2026-07-27b)**: (1) **Wall-height one-tap field** (authorized) — estimate-page "B&B wall heights (taped)" field (`SettingsRow.jsx` → `WallHeightTapeField`) writes the `batten_wall_heights` checklist (close/reopen), taped total now folds into `rebuild_lp_tab_lines` too (tab lines, not just package paths); material-list panel refetches live on the `lp-flag-checklist-changed` event; `hover-lp-run`/materialize projections extended (`color_tier`, `lp_flag_checklist`). Pinned in `test_flag_checklist.py::TestWallHeightOneTap` (247 vs 245 fixture walk). (2) **Jon Casile FULL FRESH WALK** for Howard's re-book-check: `/app/memory/casile_fresh_walk_2026-07-27.md` — header→materials→tax→labor→base→homeowner, every delta from v3 named, balances to the penny (sub_mat 20,025.74→26,468.61; homeowner 30,610.77→**40,459.16**). **NOTHING PRINTS until Howard clears the new number.** Guard: `2026-07-27 23:45 UTC · 51e9f10 · CLEAN · 1445 passed`.
- **GATE CLEARED (Q1–Q17)**. Next in Howard's ordered queue: Howard's re-book-check of the Casile walk → MODEL LEDGER panel → Part-2 Add-Dormer drag (partner's acceptance eye) → money-surfaces map.
...
## 2026-07-29 — WASTE: AREA GOODS ONLY (sealed) + QUOTE/ORDER GATES + FINAL-JOB SURFACE
- WASTE-MODEL REPORT delivered + accepted. LP length-cut rows were already clean (_waste_included architecture right); Casile live: OSC 15 stays 15, battens 194 (no 30% ride), panels 68 (30% inside, sealed).
- CATCH 1 (Howard): vinyl over-order was LIVE (79a2582 retired the Iter-79c zero-waste branch). RULING (a) LANDED: waste field multiplies AREA GOODS ONLY. All vinyl/ascend length-cut rows (OSC, ISC, starter, finish trim, J-channel, soffit-J + region-context rows) now `waste_included: True`; classifier mirrors (isCutProneItem / _cut_prone_line) narrowed to area goods; recomputeWasteQtys snaps legacy length-cut rows BACK to raw count. Detector: tests/test_waste_area_goods_only_seal.py FAILS on any length-cut row missing the flag.
- RULING (b): battens 0% waste, NO breakage allowance — pinned (bake never moves 194; no waste param on any batten emitter).
- RULING (c): default-lap DOUBLE-BAKE fixed (2,000 ft² @30%: was 372, now 286 both paths — callable waste_included flag tracks _waste_pct). PRE-PICK AUDIT: all 59 spec rows probed w=0 vs w=0.30 — lap was the ONLY inside-waste/flag mismatch. Root cause of the path's defect streak: (1) split waste ownership coordinated by a hand-kept boolean (now detector-enforced), (2) client-owned write with debounced autosave (clobber class, pinned earlier).
- RULING (d): QUOTE GATE vs ORDER GATE landed. gates.py registry: QUOTE blocking = facade_scope_unresolved_zero · area_conservation_breach · siding_family_conflict · no_siding_on_siding_job · labor_pending_contractor; ORDER blocking = batten_wall_heights · corner_locators · opening_schedule · opening_facade_attribution · porch_ceiling_implied. Enforcement: email/PDF/freeze(QR) 409 on quote gate (Accept blocks at token mint); POST /estimates/{id}/order-release refuses while order-blocked, stamps order_released. GET /estimates/{id}/gates. UNASSIGNED FLAG FAILS THE SUITE (test_quote_order_gates.py scans every literal code/kind). porch_ceiling_implied added to _FLAG_CODES (closable). FINAL-JOB SURFACE built (FinalJobSurface.jsx in EstimateEditor): both gate badges, order-tier taped inputs (batten heights, walked corners+tall, opening attribution), Release Materials button. Verified live on Casile: QUOTE blocked (labor_pending 15 rows), ORDER blocked (attribution + batten + schedule; corner_locators closed struck).
- SUITE: 1564 passed, 1 skipped (full tests -q). Stamped handback_green pending platform commit (tree was dirty during run — same invocation, green).
- CATCH 2 STANDING: when the hard batten formula lands, NAME the Casile delta explicitly (194 aggregate → ~249 hard @12" o.c., ≈+55 sticks; $38,139.13 moves) BEFORE it reaches the homeowner. Battens stay unwired until then per sequence.
- NEXT QUEUE (Howard's sequence): 3) Casile 34/5 opening attribution closure + both-counts print (NO re-import — protect the 2,064-excluded facade_scope custom; inject footprint_perimeter_ft safely) → 4) batten hard formula + 12/16/24 spacing wired (12" regional default, 8" constant retired) → 5) batten statement w/ named delta → 6) MODEL LEDGER.

## 2026-07-30 — DIMENSIONED-SKU AUDIT (all families) + REGISTER DETECTOR
- Audit report /app/memory/dimensioned_sku_audit_2026-07-29.md: 81 catalog SKUs + Mezzo/Vero buckets + ISS bands; Howard's extra column (name-constant == math-constant) applied. Findings F1–F8 HELD for his one-pass ruling (nothing built): F1 ".019 Coil (1 per 50' fascia)" name vs Q3 width-conditional 100/50; F2 fascia-coil reads m["fascia_width_in"] while spec injects "_fascia_width_in" → divisor always 100 (silent-key class, pinned as-audited); F3 'Fascia/rake up to 8" coverage' width-blind band; F4 "(2 per Sq)" J-channel suffixes claim retired math; F5 "(1 per 5 Sq)" PVC/G8 suffixes; F6 register #8 ghost names ('24" CTW'/'24" VSSFT'/'Flash tape') make that guard vacuous; F7 legacy Vertical Panel ÷32 vs live ÷40 (gated, now pinned by name); F8 stale 10' comment.
- DETECTOR: backend/tests/test_dimensioned_sku_register_2026_07_29.py — 13 pins. Unregistered dimensioned SKU FAILS SUITE; ghost register names FAIL SUITE; coupling pins (16' sticks parse==math, lap 11/sq curve, soffit 21.3, BB 4x10@40 + legacy 4x8@32 named, batten 3", fascia variants ∈ product table, nails ÷15, F2 as-audited pin, Mezzo label==bounds, Vero parser round-trip).
- PUT-model status named plainly: batten_spacing_in {12,16,24} · fascia_width_in {4,6,8,10,12} · shake_reveal_in 7–10 declared+bounded (test_spec_fields_survive_the_put_model, test_r4_bounds_and_default, test_r4_model_validator_422_shape). panel_size / wrap_trim_width_in DO NOT EXIST (baked 4×10 / 540 4") — same class, enter only on Howard's ruling.
- ID-based catalog binding SIZED (no code): ~3–4 days total; Phase 1 dual-bind (ids + name fallback + drift flag) ~1.5–2d, Phase 2 retire name binding ~1–1.5d.
- S2 region-crop vs contractor tap: RECOMMENDATION STATED — contractor tap wins (0/3 machine on known case, drawings fragment STC-1 across labels; $2–4 + 2–4 min recurring per import vs $0 taped spec-class input).
- Stamp: 2026-07-30 02:15 UTC · 2823d09 · CLEAN · 1583 passed, 1 skipped.
- AWAITING HOWARD: one-pass ruling on the audit list (F1–F7 + panel_size/wrap_trim candidacy), then STOP order honored.

## 2026-07-30 — MULTI-USE MECHANISM + GHOST-GUARD SWEEP + LP RECONCILIATION (report only, nothing built)
- Report: memory/multiuse_and_lp_reconciliation_2026-07-30.md. (1) Mechanism = LIST-PER-USE, no sum/no overwrite; live proof "3 degree vinyl": .019 Coil 5.28 + 6.28 rolls (one AMI 103954) → 12-vs-13 rounding gap; 3/4" J 63+76 (AMI 105118). Multi-block list delivered by AMI. Wall-J rake pass (1) + soffit-J (2) vs "exactly 2 passes/rake" comment — NAMED for ruling. (2) SEALED: app SKU name describes the PRODUCT; rates are emitter-sourced NOTES. Strip list specified (~1d), HELD for one-pass go. (3) Guard sweep: register-#8 three ghosts verbatim ('Flash tape'/'24" CTW'/'24" VSSFT'); NEW DEFECT: Ascend 'Inside Corners' emitter feeds a supplier-dropped row → mat=$0.00 live on 4 estimates (Casile qty 3), $11.83 orphaned in IDENTICAL_PRICES; 'j channel' marker = defensive-broad; Gutter/Downspout cleared (bind in ISS engine). (4) LP prices = BlueLinx PIT00003 2.26.2026 MILL ÷ margin (0.80/0.75/0.70/0.65), NOT Howard's Jan-2025 sheet; 1.4073 = 0.985/0.70 artifact; six outliers = Feb-2026 cost jumps (ruled KEEP-CURRENT 07-18); Vented 85.83 = 60.08/0.70, Closed 73.50 = 51.45/0.70 (PIT00003 prices Closed == Vertical Panel in every finish column); Howard's 540 4"/6" ratios don't reproduce — need his two sheet numbers (reproduce only if sheet reads 23.10/35.19). (5) ID-binding re-sized: app-minted IDs (AMI = metadata, never key; LP keys on app ID) — base 3–4d stands, +0.5d metadata, +1–1.5d optional order-layer product_id roll-up; ceiling ~5.5d.
- AWAITING HOWARD one-pass: F1–F8 + strip list + Inside Corners + register-#8 renames + multi-use order treatment + wall-J rake passes + 540 sheet readback.

## 2026-07-30 (PM) — R1 LANDED (RAKE J = 2 PASSES) + R2 SEALED · R3 TRUNCATED
- R1: soffit-J rake term 2×→1× ((eaves+rakes)÷12.5); finish-trim comment was right, derivation wrong. ONE marker (RAKE_J_DOCTRINE, "R1 ruled 2026-07-30") pins doctrine+derivations together: tests/test_rake_j_passes_2026_07_30.py (6 pins: wall-J rake=1, soffit-J rake=1, finish-trim rake=0, total==2, marker coupling, note NAMES pre-ruling qty). Delta table memory/r1_rake_j_landing_2026-07-30.md: 3 Degree vinyl 76→51 (−25 pcs, −$182 ACTIVE money); all LP-job vinyl-tab alternates named; Casile LP quote untouched. Stored lines not mutated — rebuild announces old→new on the line note. Letrick truck harness (2×eave rule) FROZEN as September evidence, named.
...
## 2026-07-31 (later) — SALES UNITS + PRICE AGE + TEST-DATA HYGIENE (all ruled, all green: 1625 passed, stamp e75f8b9)
- WRAP CONVERSION HELD per ruling: RainDrop (11.25 SQ/roll, $336.13) + House Wrap (9.00 SQ/roll, $119.11) convert ONLY after Howard uploads new price pages and confirms. Flag: catalog RainDrop $30.73/SQ vs Howard's $38.73 — unresolved, confirm at upload.
- SALES-UNIT CONVERSIONS LANDED (migrate_2026_07_31_sales_units.py, receipted, 57 lines + 4 tier docs): 2" Nails JOB→BOX (count was already boxes, ceil per whole-units, $0 delta); Downspout 6" LF→10' Stick (÷10 ceil, $2.80/LF→$28.00/stick); 3/8" Fan Fold SQ→Bundle (2 SQ, $11.06→$22.12); Pelican Bay tier-doc heal ($419.94→$209.97 — seed sync had pushed the 1/2 SQ unit but kept the per-SQ price = doubled money, caught by DB check). Money pins re-ratified: Letrick 13303.78→13314.98, Casile sub_mat 23630.84→23642.04. Coverage: nails/fan-fold Vinyl·Ascend; downspout all three tabs; ISS price book untouched (own CSV units).
- PRICE AGE BUILT: per-item price_changed_at/by stamped on ALL surfaces — tier editors (server-side diff so single-cell edits stamp), bulk panel, ISS CSV, Vero/Mezzo matrix (doc-level), LP margin ladder. Seed sync never mints stamps and carries them through section rebuilds. UI: tier-card "last price change" line, per-row date + amber STALE chip, settable threshold (branding.stale_price_days, default 90). PINNED: test_price_write_stamps_2026_07_31.py — any routes/*.py write to a price collection must be in PRICE_WRITE_SURFACES (price_age.py) and stamp, else suite fails. Checksums exclude stamps + stale_price_days (seed gate green).
- TEST-DATA HYGIENE: purged 2,332 test companies (2,039 TEST_* + 293 "Tester's Company" suite-default) + all users/catalogs/estimates + 146 resend.dev invitations. Root cause: signup derives "{name}'s Company" and no create path tagged. NOW: services.create_company tags test_artifact on TEST_*/Tester's Company names (pinned); invitations to @resend.dev tagged at insert; census+purge admin endpoints (/admin/test-data/census|purge) — purge excludes fixture_import docs. Remaining companies: Pro-Quote (Howard), GusGear, Pappans, ZZ Fixture Test Co (seed fixture). Pipeline corrected 28→23 estimates. NOTE: fixture estimates re-parented to Howard's company after seed re-apply had slotted them to zzfix (tests reach them via Howard's session).
- LP PRICING: Howard CONFIRMED 2026-07-31 that BlueLinx PIT00003 (2.26.2026) is still current — noted in lp_costs.py at QUOTE_REF.
- DEMO CHECKLIST (REMINDERS.md): LP-native mode is OFF; Howard flips it before the September demo.
- QUEUE: 1) Howard uploads wrap price pages → wrap roll conversion; 2) blind row notes (39 rows); 3) Spanish report (report only, SKU-never-translates); 4) ID binding queue.

## 2026-08-01 — THREE DOORS ACCURACY AUDIT (REPORT ONLY, NOTHING BUILT — per Howard's order)
- Full report: /app/memory/three_doors_audit_2026_08_01.md. Zero code changes. Awaiting per-finding rulings.
- LAYER MAP: dollar layer (waste/rounding/color-tier/ID-binding/_build_lines/rebuild_lp_tab_lines) is ONE shared pipeline all doors. Layer 2 aggregation is THREE copies (two `_aggregate_to_hover_shape` copies + hover direct).
- FINDINGS (blast-radius ranked): F1 gable 0.7×(photo,C4-ruled) vs 0.5×(blueprint) — 40% apart; F2 LP-pair seeds priced lines from UNAPPLIED photo run (estimates.py:306-360 Iter 99) — only unconditional human-gate violation; F3 photo apply stores no measurements + no _source → rederive 409s, photo door locked out of ONE-SHARED-REBUILD; F4 door_count never summed on both AI doors → caulk + J-blocks undercount; F5 corner COUNTS never land on photo → per-corner Q13/Q12 + never-average tall rule silently inactive on _build_lines path (pooled ÷16, the retired math); F6 photo starter falls back to EAVES (gable ends missed); F7 <4ft wall clamp injects up to 18ft silently (no flag; 4-7ft band IS flagged); F8 confidence is label-only, no backend gate anywhere; F9 register integrity — window_bottom_width_total_lf falsely CONSUMED (no consumer exists), and no Class-B register exists for blueprint/photo doors; F10 minor asymmetries.
- NEXT (post-ruling): fix 3 doors plumbing per rulings against 261 Haugh + 3 Degree with byte-identical material lists through any non-ruled plumbing change. Then Spanish buildout (P1).

## 2026-08-01 (later) — THREE DOORS: ALL TEN RULED · BUILD STEP 1 SHIPPED
...
## 2026-08-03 (later) — ACCEPT PAGE SPANISH WALKED · FILL-IN HISTORY VIEW · CORE LOCKABLE
- QUOTE OPEN RECEIPTS: PARKED by ruling (Howard 2026-08-03) — new scope, do not build before the September demo. Queued as a clean standalone after.
- ACCEPT PAGE (last customer surface) PARITY-PROVEN: walked live EN vs ES on a disposable 261 Haugh byte-copy (fixture untouched, verified un-accepted after) — Total Price $95,526.18 == Precio Total $95,526.18, figure-for-figure; Spanish end-to-end acceptance walked to the success screen ("…aceptó el estimado por $95,526.18"). FINAL-COST-ONLY pinned both sides (server exposes totals[sell] alone; page renders d.total alone; any sub_mat/labor/tax/margin in the payload fails). No line items on this surface by construction — SKU-leak impossible; pinned to demand tItem-verbatim if lines ever arrive. 7 pins: test_accept_page_spanish_2026_08_03.py.
- DEFECT FOUND BY THE WALK (6th green-and-broken on a new surface): the 2026-07-20 3D-dark commit referenced RENDER_3D_ENABLED on AcceptPage WITHOUT importing it — ReferenceError on the money-acceptance surface since July 20. Import restored; import-pin keeps the class dead.
- FILL-IN HISTORY VIEW: who/when visible ON each fill-in box — pf.lastSet chip per box ("Set by {by} · {date}" / "Definido por…") + full trail (field: prev → value · by · at, newest first) in a read-only <details>. Read-only PINNED (no saveSpec/update/onChange/fetch may originate from trail or chip). Walked live: per-box stamps + 3-entry trail render.
- SUITE FLAKES NAMED (Howard's rule: name the failure): (1) test_elevation_sheets_lbr — transient, passed standalone + in final run. (2) test_vero_pricing connection flake broke test_admin_put_roundtrip MID-RESTORE, leaving its 999.99 sentinel in the whole-sale Vero DH price — healed 999.99→287.57 via admin API, vero 7/7.
- LANDED PER HANDBACK DISCIPLINE: auto-commit 5addb1d ratified (captured ruled work mid-build); ruled commits f10a572 + bookkeeping; STAMP: "- 2026-08-03 20:11 UTC · 5aceca6 · CLEAN · [tests] · 1790 passed, 1 skipped, 3 warnings in 237.68s (0:03:57)".
- STATUS: every customer surface (estimate UI, quote modal, PDF, email, accept page) is Spanish-parity-proven; three-doors closed; catalog honest; silent-zeros gated. CORE LOCKABLE.
- NOTE: live user edits observed on EST-853809 during the session (Mountain Fern, waste 10) — user activity, not suite writes; fixtures' byte-identical pins all green.

## 2026-08-03 (night) — FROZEN WASTE FIELD KILLED (EST-803966 138-vs-106) + CEILING-DEDUP COMPOUNDING KILLED
- FOUND LIVE BY HOWARD: EST-803966 estimate panel 138 (priced), material list 106; waste 30→0 moved nothing on the estimate.
- TRACE (a-d delivered before any fix): LP tab lines bake waste at materialize (raw_qty=null, _waste_included); waste handler's client recompute reaches only raw_qty rows → zero LP rows. BOTH buttons miss it (recompute-waste skips _waste_included; re-derive unadvertised). One-emitter violation at the TRIGGER layer: ML recomposes with the live field, estimate reads the snapshot. Side-by-side: panel Δ32 the only waste diff; 440 trim stale NAME (8"→12") same frozen class.
- FIX 1: waste joins the spec-field family — updateWastePct → same /rederive door (siding-kind; lp_smart only when derived rows exist — a waste change never materializes lines, dollars on apply). Client raw_qty recompute kept for instant feedback.
- FIX 2 (found en route, pre-existing since 2026-07-28): ceiling-dedup fold deducted duplicate_sqft from the ALREADY-deducted soffit while /rederive persists scoped back → −40 sqft COMPOUNDING per call on 261 Haugh (423→383→343→…). Fold now deducts from the pre-dedup base (_soffit_sqft_hover) — idempotent, 5×-pinned stable at 423.
- PROOF (computed, both estates): EST-803966 waste 30% → panel 138 on BOTH surfaces; 0% → 106 BOTH; walked live in the UI (header total moved 74,033.02↔80,780.26); healed + settled at Howard's 0/106; 440 trim name healed to 12". 261 Haugh: 30%→68/68, 0%→52/52, 30→0→30 byte-identical True, settled at his 30; soffit stable 423. Rederive also healed EST-803966 compare-tab rows to today's rulebook (.019 Coil 7→13 vinyl/ascend, iss gutter rows composed, zero-qty scaffold rows) — door semantics, not the LP money tab.
- PINS: test_waste_rederives_live_2026_08_03.py (8) — rederive reads the current field, both surfaces one panel qty, byte-identical round trips (803966 shape + Haugh shape), dedup no-compound 5×, frontend wiring pin incl. lp_smart gate.
- STAMP: "- 2026-08-03 21:42 UTC · c561c21 · CLEAN · [tests] · 1796 passed, 1 skipped, 3 warnings in 363.60s (0:06:03)".

## 2026-08-03 (later night) — DOORS SINGLE-FAMILY · ESTIMATES SELF-CONTAINED · PHANTOMS DELETED
- REGISTERED RULING: "doors are single-family and estimates are self-contained by ruling 2026-08-03; cross-family fill is post-September." (comment pinned in routes/estimates.py + detector test_doors_single_family_2026_08_03.py, 6 pins.)
- CAUSE REMOVED: Hover + Blueprint import doors auto-called /pair, seeding a cross-family sibling (LP→siding, vinyl→windows — both directions). Blocks deleted; a door writes ONLY its own estimate. Own-family output untouched (door tests all green).
- PAIRING RETIRED: /pair, /pair-lp endpoints removed; PairToLpButton deleted; dashboard pair badge + list pair-number resolution removed.
- READERS NAMED + SEVERED (Howard's pre-delete check): (R1) _load_run paired-latest fallback — LP composition could silently derive from the SIBLING's run (same class as the killed LP-pair-seeds-from-another-run defect); (R2) LpMaterialListPanel paired-run fetch. Both severed. Display-only pair badge was the only other reader (no numbers). No pricing/pairing/rebuild read a BLANK (blanks carry no runs bindable at compose time... except red-house, handled next).
- RED HOUSE MADE SELF-CONTAINED: EST-910869-L composed via the revoked paired-latest exception from sibling 673707d5's runs. -L now carries its OWN cloned governing run (c2002212…-selfL); preview 200, binding latest-run. The named exception in test_material_list_reads_only_its_own_estimate REVOKED and re-pinned as severed.
...
## SESSION 2026-08-04 (part 2) — DEMO BLOCKERS CLEARED (Howard's rulings, one pass)
GREEN STAMP: `2026-08-04 04:03 UTC · 4faa888 · CLEAN · 1804 passed, 1 skipped` (handback_green_log.md, verbatim).
Demo house NAMED: **Letrick Ranch — LP Photo Demo** (DEMO-LETRICK); id changes on every reset — resolve by demo_key.
1. **LABOR ROWS NAMED (rule, all estimates)**: clean up/haul away + Cap window + Cap entry door are the CONTRACTOR'S labor, never supplier catalog gaps (they sit in MISC_LABOR_ROWS, lp_conventions.py). Readiness reads "LABOR PENDING — contractor to price: {row}" (kind `labor_pending_row`, registered quote-tier BLOCKING in gates.py). Generic `unpriced_row` re-ruled BLOCKING ("the PRINT-BLOCK working, not a bug. Keep the rule."). Aggregated LABOR UNDECIDED line stays non-blocking (2026-07-29 ruling intact).
2. **DEMO fixture only**: reset pre-fills demo labor rates (DEMO_LABOR_RATES in demo.py: haul-away $350/JOB, cap window $65/ea, cap entry door $85/ea, lab_src human) — gate clears BECAUSE labor is priced. Labor total $1,105.00.
3. **CHIMNEY AMBER RATIFIED**: `corner:isc:chimney-chase-left-edge-meets-back-wall` = PRESENCE ratification (never a measurement). Evidence: photo 4 "chase projects from back wall ~18.5–24.5 ft", photo 5 detects all four chase corners, Howard's sealed ledger "2 ISC at the chase". Reset seeds it verified with note "ratified per Howard corner ledger — 2 ISC at chase confirmed" + corner.verified event. DOCTRINE HELD: a dimension-class field-verify still gets a real number or stays open.
4. **MARGIN**: reset seeds pricing_mode margin + margin_pct 30.0. Sell $18,338.69 = base $12,837.08 ÷ 0.70, penny-exact on header + summary + quote. UNDEFINED% label fixed (`?? 0` in TotalsSummary, ratified auto-commit 0f36116 → memory/ratified_commits.txt).
5. **GATE CHIP TRUTH SEALED**: `_readiness_items` stamps EVERY item with tier+blocking from the gates.py registry — chips (/gates), modal (QuoteModal reads `i.blocking`), and assert_quote_gate consume the SAME flags. ORDER-tier items (field-verify ambers, taped flags) ride visible but NEVER gate the quote (amber = offer, never a gate — sealed pins test_one_money_surface/test_run_archive restored after the in-progress blanket-blocking broke them). Pins: test_gate_chip_truth_2026_08_04.py (demo id resolved live by demo_key).
6. **STAGE NOISE KILLED**: CatalogSyncBanner respects human labor overrides (lab_src "human" never flagged stale, apply never overwrites — human overrides are absolute); demo reset stamps lp_source_run_id so pending-runs banner reads the truth (run IS applied). Fork-boundary seam honored (demo.py lp_costs import removed).
7. **RE-WALK PROVEN (screenshots)**: quote opens clean (0 PRINT-BLOCKED, Email+PDF live), QUOTE GATE — CLEAR + ORDER GATE — CLEAR chips, 30% margin of sale, no banners, all 21 rows visible incl. labor rows, Total $18,338.69.
MORNING-OF STATE (consolidated): margin auto-stages at Contractor 30% on reset · demo reset in PROD revokes+reprints QR links (reprint AFTER reset) · LP-native mode confirmed ON in prod (do not re-flag) · demo run is the frozen clone (no live AI credits needed for the staged walk; rotate Anthropic key per REMINDERS before any LIVE run) · clean browser + fresh login morning-of.

## SESSION 2026-08-04 (part 3) — SPANISH DRY RUN ON THE DEMO FIXTURE (screenshot-proven)
GREEN STAMP: `2026-08-04 11:27 UTC · 6a48b11 · CLEAN · 1804 passed, 1 skipped` (verbatim).
Labor rates $350/$65/$85 RATIFIED by Howard — stand as seeded.
ES walk on DEMO-LETRICK, EN==ES penny-for-penny: header $18,338.69 · MATERIAL $11,732.08 · MANO DE OBRA $1,105.00 · COSTO BASE $12,837.08 · Precio Total $18,338.69. SKU names verbatim EN inside ES chrome (38 Series Lap…, Cap window, clean up/ haul away job debris). Chips: PUERTA DE COTIZACIÓN — LIBRE / PUERTA DE PEDIDO — LIBRE. Readiness: "Todo listo…". Quote opens clean in ES (0 print-blocks, ENVIAR + DESCARGAR PDF live). Chimney amber reads ✓ CONFIRMADO in ES chrome.
SHIPPED for the walk: GateBadge/FinalJobSurface/QuoteModal print-block/ReadinessPanel fully keyed EN+ES (gates.*, quote.printBlocked*, readiness.* in dictionaries.js); ReadinessPanel footer re-worded to the unified truth (blocking items stop the customer quote). Catalog maps: "Misc. Labor and Material" + LP family sections (brand verbatim per Ascend precedent), units Stick→TRAMO/Tube→TUBO, ⌁ derivado chip, Readiness button → PREPARACIÓN.
CUSTOMER-SURFACE CATCH & FIX: ES email subject said "Su estimado de vinil" on EVERY siding family — an LP job mis-sold as vinyl. Now "Su estimado de revestimiento" (email.subjectWithCustomer/Generic ES).
NAMED RESIDUAL EN BLEED (CONTRACTOR chrome only, NOT customer surfaces — awaiting Howard's priority): intake tiles (HOVER PDF / AI Photo Measure / Read Blueprints), waste-card sublabels + soffit selector + "Sell = Base ÷ …" formula line (SettingsRow), deep takeoff/field-verify panel provenance strings (AIMeasureButton: "derived line(s)…", PER-WALL TAKEOFF table headers, TAPED DIMS block), pending-runs banner text (JobInfoPanel). Backend-built readiness labels also EN (only visible when items stand — demo shows none).

## SESSION 2026-08-04 (part 4) — SPANISH FULL SWEEP (Howard: "FULLY FUNCTIONAL IN SPANISH — SWEEP EVERYTHING")
GREEN STAMP: `2026-08-04 13:38 UTC · 3ea93e6 · CLEAN · 1810 passed, 1 skipped` (verbatim; includes 6 new detector pins).
1. **CONTRACTOR CHROME SWEPT**: intake tiles (IMPORTAR HOVER / LEER PLANOS / MEDICIÓN IA), waste card + RECALCULAR DESPERDICIO, soffit selector, trade specs (fascia/batten/panel/wrap/shake), B&B wall-height tape field, Venta = Base ÷ (1 − %) line, FieldVerifyCard (despiece por pared · control con cinta · dimensiones medidas + EL-x FRENTE/ATRÁS/IZQUIERDA/DERECHA chips + taped-dims provenance + IMPRIMIR TODO), LpMaterialListPanel (derived note, pending count, compare card, flag verbs), JobInfoPanel (pending-runs banner, tiles, Editar). Key prefixes: sr.* fv.* lpp.* jip.* intake.* mls.* ars.* in dictionaries.js.
2. **QR PAGES BILINGUAL**: /m/ + /r/ honor ?lang, carry PublicLangToggle (EN|ES), es-MX dates; sections/units translate (Stick→TRAMO, Tube→TUBO); **SKU names and COLOR names VERBATIM** (color-Spanish remains ruled OUT). Frozen accuracy-report BODY renders exactly as sealed with ES note "…documento original en inglés" — never a silent re-render of a frozen artifact.
3. **ACCEPT REHEARSED LIVE in ES**: quote emailed from ES modal (status sent, token minted via real email door incl. assert_quote_gate), homeowner accept walked to success — "Listo, Letrick… aceptó el estimado DEMO-LETRICK por $18,338.69" — then demo reset restored staged state (demo est id rotates every reset; resolve by demo_key).
4. **DETECTORS EXTENDED** (test_spanish_full_sweep_2026_08_04.py): QR no-hardcoded-english-between-tags, {l.name}/{l.color} verbatim pins, ?lang+toggle pins, named-key EN/ES presence, SKU-leak scan, frozen-body-sealed pin. Source pins updated to follow keys: fv.defaultDepth (dormer), fv.gables.sub (gable), fv.door.* (3D pin5); markup label spelled (1+{pct}%) so the one-waste-emitter signature never reads a display string as math. ES subject fix from part 3 stands ("revestimiento", never "vinil").
5. **Penny parity after sweep**: EN + ES both $18,338.69 everywhere (header, summary, quote, accept); no SKU/color/tier/price moved. NAMED REMAINING (server-authored, EN by doctrine): amber locator strings, run/geometry provenance labels, readiness item labels, color_flags — visible only inside contractor provenance rows.

## SESSION 2026-08-04 (part 5) — DOOR SEPARATION CLASS CLOSED: LINE RESTORE, ALL DOORS, BOTH DIRECTIONS
GREEN STAMP: `2026-08-04 15:09 UTC · c48509b · CLEAN · 1815 passed, 1 skipped` (verbatim).
ROOT (two halves, same class): (1) HoverImportButton apply kept ["vinyl","ascend","lp_smart"] on lp_smart kind — the only door that never got THE CUT (2026-07-14); (2) backend /rederive scoped siding-kind but wrote ALL families wholesale on lp_smart (re-contaminated Jon Casile mid-suite — caught live by the new invariant pin).
FIX: Hover door under THE CUT via shared `applicableRestoreLines` (lp_smart merges nothing — hover-lp-run rebuilds server-side; siding=vinyl+ascend; windows=windows); preview modal + Apply count read the SAME filter (87-line over-count dead; hov.cutNote/hov.applyLp EN+ES); pairing residue (pairedLines/wastedPaired) deleted from Hover + Blueprint; /rederive lp_smart branch writes LP family only, carries service tabs (iss) verbatim + human/flagged rows.
PIN EXTENDED (test_doors_single_family): 5 line-surface pins + LIVE DB INVARIANT (fails on any cross-family qty>0 line, kinds lp_smart/windows/siding, respecting cross_family_flag survivors). ISS gutter tab = service overlay, outside the family rule.
SIX ESTIMATES STRIPPED under the absolute guard — LP surface sha256 BYTE-IDENTICAL through every touch (new clean LP-only anchors):
- EST-510771 doug jones: -26v -25a | LP 21 lines $9,560.87 | 15524ec592a25c22
- EST-523061 Jon Casile: -29v -28a -6w | LP 27 lines $23,642.04 | d9ea4d4e842c30b9
- EST-562488 3 degree rd: -30v -29a -5w | LP 26 lines $49,958.89 | e5535f3c05bdf5e9
- EST-109465 3 degree 7-28 8am: -25v -24a -5w | LP 22 lines $26,958.97 | e10b288d03025e22
- EST-630295 3 degree 7-28 1pm: -28v -27a -5w | LP 25 lines $48,679.36 | beaed9b1ffba7d81
- EST-803966 3 degree 8-3 5pm: -26v -25a -5w | LP 23 lines $49,950.16 | 93b25a910a298c8e
FLAGGED, NOT DELETED (Howard's guard — awaiting his ruling): 24 legacy typed-dollar rows restored with `cross_family_flag: "howard-2026-08-04"` — window install fees ($1,846.15 story fee, $170 pocket install, $669.63 SGD, $20 cap, $150 measure, $125 disposal) on Casile + two 3 Degree estimates; Dutch Lap $250 lab + soffit/gutter/downspout rows on doug jones; Ascend Lap $250 lab (qty 0) on 3 degree rd. Backups + full row receipt: memory/backups/20260804_strip_cross_family/.
261 Haugh + DEMO-LETRICK verified clean/untouched. The 87-line restore was never applied (/measure/map is pure compute; closing the modal wrote nothing).

## SESSION 2026-08-04 (part 6) — HOVER MODAL COLLAPSE (Howard's ask) + THIRD RE-CONTAMINATION DOOR CLOSED (found by the suite)
HOWARD'S ASK (display-only): "VERO WINDOW OPENINGS — STYLE GUESS" (30 rows) collapsed BY DEFAULT in the Hover import modal.
1. **OPENINGS COLLAPSED BY DEFAULT** — header reads "Window openings (N) — tap to review styles"; one click expands the FULL editable table (same per-row style pickers, same remove buttons). Unresolved style guesses surface as an "N need review" badge ON the collapsed header — the fold never buries a flag. Sanity warnings got the same fold ("Sanity check · N warnings — tap to review"); any future non-info/warn level stays visible while collapsed. Collapse state resets per import/restore. PINNED (test_hover_modal_collapse_2026_08_04.py): apply() never reads either collapse boolean — applied payload byte-identical open or closed. Screenshot-proven both states.
2. **THIRD DOOR CLOSED (found when the suite re-contaminated Jon Casile mid-run)**: rebuild_lp_tab_lines emits EVERY tab; /rederive filtered (part 5) but hover-lp-run and lp-package/materialize wrote the rebuild WHOLESALE — every materialize re-landed vinyl/ascend/windows rows on the LP estimate. NOW: ONE COPY `scope_to_lp_family` (hover.py) shared by all three doors; carries service tabs verbatim + human/flagged rows; SPEC-RENAME BINDING (digit-normalized, same class as the tier sweep) drops machine LP rows the respec consumed (4x10→4x8 panel double-surface). PINNED: test_lp_materialize_doors_scope_through_the_one_copy in test_doors_single_family.
3. **RESTORE WASTE CLOBBER KILLED** (EST-536665 live: panels 138→117): the "Restore HOVER lines" apply wrote the import-time generic 10 prefill over the family-defaulted field (B&B 30). Reconciled with the sealed one-rule: the PREFILL resolves to the estimate's OWN field on restores (every kind, no family branch at apply). PINNED (test_hover_restore_waste_guard_2026_08_04.py).
4. **DB REPAIRED via established doors** (backups: memory/backups/20260804_race_recontamination/): Jon Casile stripped twice via /rederive — LP surface sha256 byte-identical both times; EST-536665 stripped + waste restored to 30 (materialize's family default; only diff vs pre-clean: two qty-0 Tear-Off/Dumpster template rows). Casile's 6 typed-dollar windows rows ($1,846.15/$170/$669.63/$20/$150/$125) had lost their flag to the wholesale write — RESTORED with cross_family_flag="howard-2026-08-04" (never silently deleted).
5. **OPERATOR NOTE (lesson recorded)**: two pytest runs must NEVER overlap — concurrent suites race on the live fixtures (this session's first contamination was agent-caused: a backgrounded full suite + a foreground 3-file run). memory/handback_green_log.md carries the stamp.
FULL SUITE: 1824 passed, 1 skipped (sequential, clean).
NOTE FOR HOWARD: if a browser tab has been open since before yesterday's fix, hard-refresh it once — a stale bundle can still write the old way.

## SESSION 2026-08-04 (part 7) — PRINT CRASH KILLED (_lang ReferenceError) + THE UNDEF CLASS SWEPT
HOWARD'S REPRO (hard-refreshed tab, live bundle): PRINT on the HOVER Lines Restored modal threw `ReferenceError: _lang is not defined` at renderLinesHtml — the Spanish sweep called translateNote(l.note, _lang) in the module-scope renderer while _lang lived inside printTakeoff(). Every takeoff-modal PRINT with a noted line crashed; 1,824 green because no test EXECUTED the print render.
1. **FIX**: renderLinesHtml takes `_lang = "en"` as a DEFAULTED parameter (never crashes regardless of language state); printTakeoff passes its resolved lang down.
2. **CLASS SWEPT**: ESLint no-undef across ALL of frontend/src — exactly ONE member left (_lang × 2 sites, same file); RENDER_3D_ENABLED was the first, fixed prior. Zero others — named and verified.
3. **PIN — REAL EXECUTION, NOT IMPORT** (test_print_smoke_2026_08_04.py + tests/print_smoke/): esbuild-bundles the print builders and EXECUTES them in node (DOM-stubbed) per surface per language: printTakeoff (HOVER restore modal, ISS modal, Blueprint modal, AI Measure modal — all kinds), buildMaterialListHtml (estimate page + ISS editor), buildLpMaterialListHtml (LP Material List panel). Plus an ESLint no-undef sweep pin over the whole src (keeps the CLASS dead, not just this member). Proven red on the planted bug before sealing. Quote/customer PDFs print server-side (WeasyPrint, suite-covered); elevation sheets print the React page itself.
4. **WALKED LIVE, BOTH LANGUAGES**: PRINT clicked on the Hover restore modal on EST-853809 (siding) and EST-803966 (lp_smart) in EN and ES — popup opened, ~12K chars rendered, window.print fired, ZERO js errors; printouts screenshot-proven (notes translate — "el total medido gobierna" — SKUs verbatim).
FULL SUITE: 1827 passed, 1 skipped.

## SESSION 2026-08-04 (part 8) — DOOR SEPARATION ON PAPER (the third lesson)
HOWARD'S REPRO: EST-536665 clean LP-only on screen, but the printed HOVER Takeoff Preview carried "VINYL SIDING — 28 LINES". ROOT CAUSE: the print built its view from the RAW ALL-FAMILY MAPPER OUTPUT (result.lines) while the modal used applicableRestoreLines — print and screen read different sources.
1. **FIX AT THE EMITTER**: printTakeoff now scopes inside itself (`scopePrintLines` + PRINT_FAMILY_TABS) so NO caller can print cross-family: lp_smart prints the ESTIMATE'S OWN materialized lines (qty>0, never the mapper's default-profile LP variant); siding prints vinyl/ascend only; windows only windows; iss only iss. Byte-identical on in-family lines — only removes rows that never belonged.
2. **PARALLEL SWEEP (all surfaces reading mapper output, named)**: printTakeoff callers — Hover restore modal, ISS Hover modal (already iss-only payload), Blueprint modal, AI Measure modal — ALL covered by the emitter guard. On-screen modal line list uses applicableRestoreLines (scoped ✓, screen/print now agree). TakeoffReconCard reads raw lines but is kind-scoped by construction (LP_RECON_ROWS vs VINYL_RECON_ROWS lookups). AIExtractionDebugModal renders raw extraction BY DESIGN (debug surface, not customer paper).
...
## SESSION 2026-08-05 (part 10) — BONI RULINGS LANDED (EST-190197): EAVES ACROSS ALL ROOF PLANES + INTEGRAL-J + EAVE/PORCH-J (family-scoped)
1. **EAVES PLANE-SUM (Finding 1)**: eaves summed across ALL roof planes (main+garage+porch) with the porch ceiling riding the same read; Iter-57w wall-derived override stands down when a plane sum exists. Cascade ACCEPTED by Howard: gutter 167 EXACT, elbows 14 EXACT, end caps 12 EXACT, soffit 40 (porch ceiling filled), fascia 249, finish trim 19, pipe clips 28, sealant 5, soffit-coil 3. Hangers held at 90 (Ruling 2 — 2-ft spacing untouched).
2. **INTEGRAL-J TOGGLE (Finding 2)**: per-job flag, default OFF (default-off is byte-identical, pinned). ON touches EXACTLY four lines with provenance: wall-J (window perimeter out), caulk (doors only → 5), wrap coil (window term out), Cap window → qty 0 VISIBLE with note ("a visible 0 is a stated decision, a missing row is silence" — Howard).
3. **EAVE/PORCH-J RULED (the +16)**: wall-side eave receiving channel + porch-ceiling channel (3 sides) are REAL on VINYL and ASCEND (soffit panels tuck into a wall channel) — NEVER LP SmartSide. Added to `_j_channel_compute` on EVERY house, ungated; LP carries no wall-J line structurally. Porch channel: plane-aware (15 + 2×(150/15) = 35 LF) with 3×√sqft square fallback. BONI ACCEPTANCE MET: 168 + 167 + 35 = 370 LF ÷ 12.5 = **30 pcs EXACT vs installed 30** — both findings resolve end to end.
4. **PINS**: test_boni_rulings (7 tests incl. J==30 + family-scoping pin that fails if eave/porch-J ever appears on an LP line or leaves a vinyl/Ascend one). J-formula unit pins updated to the ruling (typical 18→26, garage 25→35, clamp 1→5, rakes-only 6→18, LETRICK 16→25 vs actual order 20 — overshoot NAMED). Region-split gets its own "— eave/porch soffit channel" context line.
5. **BLAST RADIUS (named)**: stored estimates move NOTHING until rederived. Only 2 vinyl estimates carry a wall-J line: EST-979583 (J 63) and EST-853809 (J 64, eaves 308.25 → on rederive +~25 pcs). LP anchors (261 Haugh, 3 Degree, demo) byte-identical — suite proves it.
...
## SESSION 2026-08-06 (part 16) — READINESS BADGES RENAMED + DONE-RUN RESTORE NEVER EXPIRES
- (a) Badges renamed (Howard ruled): "QUOTE GATE"→"Ready to quote?", "ORDER GATE"→"Ready to order?" (ES "¿Listo para cotizar?"/"¿Listo para ordenar?"). Swept remaining gate-language values: gates.quoteBlockers→"Not ready to quote — these block…", gates.orderItems, gates.orderBlocked→"Not ready to order", readiness.kind.quote_gate→"Quote readiness"; ES equivalents; BLOQUEADA→BLOQUEADO (gender fix after losing "puerta").
- (b) Banned-word pin: test_the_word_gate_never_reaches_a_contractor scans ALL dictionary VALUES for \bgates?\b (EN) + "compuerta"/"puerta de cotización|pedido" (ES). Keys, internal code, logs, 409-body keys keep "gate". ES "puerta" stays legal for real doors (door-knock, sliding doors, Three-Doors metaphor). Plus test_readiness_badges_are_plain_questions.
- (c) RESTORE reload bug: BlueprintMeasureButton hid RESTORE + TAG PROFILES when run.age_seconds ≥ 1800. Ruling: a paid done read NEVER expires. Removed expiry for done runs (error/running keep 30-min freshness so dead workers don't nag). Data confirmed persistent: backend serves TTL-reaped docs from CUT archive (fixture_runs, no TTL) — restore works at any age. Hover has NO age gate; the stale pattern came from AIMeasureButton (photo door) — fixed there too (done/running always surface). Pins: test_done_blueprint_reads_never_expire + photo twin in test_blueprint_restore_parity_2026_08_06.py.
- VERIFIED LIVE on Boni EST-190197: hard reload with a weeks-old done run → RESTORE BLUEPRINT READ + TAG PROFILES visible; click → full Takeoff Preview modal WITH Read-Back Card; badge screenshots EN + ES clean.
- Stamp: 2026-08-06 18:30 UTC · CLEAN · 1868 passed, 1 skipped. Preview only.

## SESSION 2026-08-06 (part 17) — INTEGRAL-J TOGGLE WIRED FOR REAL
- Howard: "the toggle I built for this house fails on this house" (Boni EST-886440 / 1f8b8b60, fresh 8-6 run). TRACED — no server error anywhere (PUT 200, rederive 200, derivation with flag ON clean): the toggle was a NO-OP, the F2 silent-strip class on BOTH halves at once:
  (1) buildPayload's PUT whitelist dropped windows_integral_j — checkbox toasted "Saved", flag never persisted (reverted on reload);
  (2) /rederive's live-override list ignored the flag — rebuild always derived OFF from the stale stored doc.
  (3) residual: the zeroed Cap window row (ruled VISIBLE at 0) was being eaten by the PUT's qty-0 filter — now survives via its integral-J provenance note.
- The "something went wrong" Howard saw = the generic formatApiError fallback (transient PUT failure, unreproducible; every server path 200s). Folded into the plain-language discipline anyway: fallback now "That didn't go through — check your connection and try again."; spec-save rederive failures toast "Your setting saved, but the material lines didn't update — press RE-DERIVE…" (err.spec.lines, EN+ES).
- VERIFIED LIVE on EST-886440, full browser cycle: toggle ON persists through hard reload; J 67→35 (window perimeter out; toward ~32 — remaining delta belongs to the HELD eave-geometry findings), caulk 29→4, wrap coil 6→2, Cap window→0 visible with note; quote blockers stopped naming Cap window; OFF restores byte-identical. Flag left ON per Howard (he needs integral-J ON to grade geometry).
- Pins: test_integral_j_toggle_2026_08_06.py — PUT whitelist carries the flag, rederive override list accepts it, qty-0 Cap window survives the filter, plain-language toast wired, four-line on/off round trip at the derivation layer.
- HELD per Howard (do NOT fix until he sends findings with integral-J ON): gutter/eaves 198 vs installed 167 (plane-sum swung past real — suspect shared eave double-count where garage meets main); outside corners 8 vs installed 11 (garage-projection corners uncounted, one dropped).
- Ratified 0c7ce7f (platform auto-commit that captured the ruled badge/restore work). Flaky-under-load: test_hover_import_async::test_status_eventually_done_or_error failed once in a full run, passes standalone — transient, watch for recurrence.
- Stamp: 2026-08-06 21:07 UTC · f37cdfb · CLEAN · 1874 passed, 1 skipped. Preview only.

## SESSION 2026-08-07 (part 18) — BONI 8-6 SEND: RULINGS 1-3, STOP AVERAGING CORNER HEIGHTS, GUTTER RUN INVENTORY
- PURITY RIDER honored: no figure from the send entered code; detector extended over the new corner/gutter helpers (pure, idempotent, non-mutating, no evidence constants — barred-numeral scan); Boni NOT patched (re-derived, honestly unchanged where the stored read lacks the new fields).
- R1 REGISTRY ROUND-TRIP: test_spec_registry_roundtrip_2026_08_06.py enumerates TRADE_SPEC_FAMILY_REGISTER and walks every field UI → PUT whitelist → PUT model → /rederive projection → override list → derivation consumer; unregistered spec controls fail the suite; photo fill-ins walk their own contract. FIRST CATCH: lp_soffit_type was missing from the rederive projection + override list (spec silently reverted to default on rebuild) — fixed. windows_integral_j + waste_pct registered (families + rulings recorded).
- R2 QTY-0 FILTER DIES EVERYWHERE: buildPayload keeps ANY qty-0 row carrying a note (not a Cap-window exception). Verified live: Dumpster/Tear-Off print at 0 instead of vanishing.
- R3 SERVER-SIDE DETAIL: RequestValidationError handler logs method/path/field/validation/layer to the ring buffer — log-only, 422 shape unchanged. Pinned via direct handler test.
- CORNERS — STOP AVERAGING (ordered): blueprint prompt returns outside_corner_heights_ft (per corner, TALLER of its two walls, run to the EAVE — gable never area÷width); merge accepts only with an accepted walk covering the count; aggregation scopes _osc_corner_heights_ft + _osc_heights_source. Derivation: per-corner ceil(own height/stick) min 1 — vinyl/Ascend 12.5' stick, LP 16' stick, stick length STATED on the note as the second-piece threshold (closes corner_locators). Taped heights (human) still win over door reads. Undimensioned corners hold 1 stick and KEEP THEIR FLAG. Pooled fallbacks byte-identical. Provenance per door: blueprint dimensioned / hover callouts / photo AI-VERIFY (never presented taped). CORRECTIONS honored: no 11-stick target (sticks≠corners), garage-projection corner trace STOPPED (count 8 confirmed correct).
- GUTTER 198-vs-167 MECHANISM (reported, not tuned): eaves_lf = roof-plane sum (main 116 + garage 50 + porch 32 = 198 on Boni); the plane census lists the garage bump-out eave beside the run it already sits inside and porch eave sides carrying no gutter; the gutter line consumed raw plane-summed eaves. CONVENTION: gutter consumes the door's RUN INVENTORY (prompt: continuous facade runs, counted ONCE) and DISCLOSES the run list on the note; eaves stay the soffit figure + unchanged fallback. Boni's stored read predates runs → gutter stays 198 until a re-read; DISAGREEMENT REPORTED, not tuned.
- SOFFIT-J TRACE (report only, ordered): Boni's line derives AFTER R1 — (eaves 198 + rakes 114) ÷ 12.5 = 25 pcs vs installed 35; delta 10 pcs ≈ 125 LF unseen. Candidate mechanism: porch-ceiling perimeter J is in NO formula (the soffit PANEL line pools the 99 sqft; its perimeter channel is uncounted); eave over-count interplay widens (not narrows) the honest gap. NO fix without a ruling.
- FLAKE CLASS observed: live-HTTP tests (test_lp_master_sheet_binding, test_hover_import_async, test_one_money_surface, test_accuracy_report_share) intermittently error under full-suite load through the preview ingress; each passes solo. Watch for recurrence; consider a ruling on retry discipline if it persists.
- Integral-J left ON on EST-886440 (ungraded).
- Stamp: 2026-08-07 12:57 UTC · e6405c0 · CLEAN · 1892 passed, 1 skipped. Preview only.

## SESSION 2026-08-07 (part 19) — SEQUENCING RULINGS (no build)
- DEMO SCRIPT CARD: CLOSED — dropped from the queue permanently (Howard's partner writes it; never ask for the one-liners again).
- STANDING RULING — ACCURACY OUTRANKS SEPTEMBER: the date is Howard's to move; never compress scope to a calendar, never sit on a finding to protect a date, never suppress a flag to make a demo look cleaner ("a flag firing correctly in front of the BDM is the product working"). Correct-and-slow beats fast-and-green.
- DEMO ACCEPTANCE BAR: DRAFT ONLY — commented on, NOT ruled, NOT built to. Agent's commentary delivered 2026-08-07 (see chat): (a) needs "passes" defined without making second opinions a pass/fail standard; (b) cheapest hardening = per-fixture sha256 manifest naming the drifted line; (c) most expensive — needs the installed list as a data fixture + a reconciliation artifact (MATCH / DELTA-EXPLAINED / DELTA-OPEN) + a named provenance grade; (d) cheap and should lead — every-line-has-an-explaining-note walk. Suggested adds: flags-fire census on the demo job; registry+purity detectors named in the bar; gate truthfulness. AWAIT Howard's ruling before any of it becomes code.
- BONI PROVENANCE: stays DELIVERED-MATERIAL (strong evidence, not proof) until Howard says FIELD-VERIFIED. No ruling gets sealed on Boni deltas alone.

## SESSION 2026-08-07 (part 20) — ACCEPTANCE BAR SEALED + OPEN ITEMS 2-4
- ACCEPTANCE BAR SEALED (Howard, build order d→f→a→b→e→g→c; h scoped-not-built): PRODUCT-CORRECT = (a)(b)(c)(d)(f); DEMO-SAFE = (e)(g)(h). (a) as agent rewrote it VERBATIM: deltas vs second opinions RECORDED with cause, NEVER SCORED (battens 265-vs-465, 540 100-vs-142 stay recorded deltas). (b) manifest must NAME the drifted line + amount. (c) grade ruled: DELIVERED list qualifies; delta vs it = EVIDENCE (substitution/overage named); test asserts explanations exist, never that numbers match. (h) BROWSER WALK on Howard's machine/network stamps READY — suite gets no vote; awaits partner's demo script; DO NOT build yet. DEMO RUNS ON LETRICK + RED HOUSE, not Boni — (c) on Boni never certifies the demo job. NONE of the bar lines built yet — per-line costs delivered, Howard sequences.
- ITEM 3 FIXED — suite determinism: flake root cause = preview-ingress hop. Suite traffic now localhost:8001 (conftest, before collection; TEST_API_EXTERNAL=1 for deliberate external e2e). Test cookiejar taught the browser localhost-secure-context rule (server cookie flags untouched). 0 flakes, runtime halved (~2:00). Old PRD watch item RESOLVED.
- ITEM 2 FIXED — ring buffer does NOT survive restart (in-memory); WARNING+ evidence now also lands on rotating disk sink /var/log/pro_quotes_warnings.log. Pinned.
- ITEM 4 BUILT — Read-Back Card now surfaces per-corner heights (undimensioned flagged) + gutter run inventory, EN+ES; absence named ("re-read carries the runs"). Grade-before-apply order honored; Boni re-read awaits Howard.
- ITEM 1 REPORTED — lp_soffit_type blast radius ZERO (all estimates store default "mix"; red house "" coalesces to mix); nothing to reopen. DEEPER FINDING awaiting ruling: vented/closed steer lives ONLY in frontend apply (steerLpSoffit); no backend rederive consumer — a non-default choice would not survive a rebuild. Ownership ruling requested (move steer into backend rebuild, ~small) before any code moves.
- ANSWERS OWED DELIVERED: Boni = VINYL family (kind siding, vinyl tab priced) → 12.5' stick. Soffit-J 35 = BOUGHT per Howard's own DELIVERED-material provenance; hung count unknown to the app; both sides are purchase counts but theirs may carry crew overage — reconcilable delta not established as 10.
- DEMO SCRIPT CARD: stays CLOSED. Integral-J stays ON on EST-886440.
- Stamp: 2026-08-07 14:52 UTC · 40bf538 · CLEAN · 1898 passed, 1 skipped (~2 min). Preview only.

## SESSION 2026-08-07 (part 21) — SOFFIT-J RULED IN, BAR d/f/a/b BUILT, STEER BACKEND, INGRESS CADENCE
- SOFFIT-J (ruled): porch-ceiling J = FULL PERIMETER, 4×√ceiling-area (square-assumed, disclosed), all three doors, vinyl/Ascend only. Guardrails: porch-only scope (soffit-J single-run untouched); note prints the convention; per-door identification NAMED (blueprint plane / human / unlabeled-VERIFY) + "no porch identified" flag — never silent either way. EAVE SINGLE-RUN on the soffit-J line was ACCIDENTAL-RIGHT → SEALED 2026-08-07 (note + pin). Boni wall-J pins updated by ruling: 32→33 (porch 28→40). 25-vs-35 line NOT closed — reconciles at Howard's site walk.
- SOFFIT STEER BACKEND (ruled): _steer_lp_soffit in the rebuild via scoped _lp_soffit_type — vented/closed survives every rederive. R1 ANSWER: the round-trip had asserted TRANSPORT ONLY; consumer check now demands a derivation-side read (est.get / scoped _key) — lp_soffit_type passes only because the steer exists.
- BAR (d) BUILT: every emitted line explains itself (inputs + convention, formula chips) — walk surfaced 13 prose/empty notes (Tear-Off, Dumpster, fees, iss Gutter, Ascend default…) → all chip-styled; iss Gutter now consumes the run inventory.
- BAR (f) BUILT: standing detectors named + presence-pinned. BAR (a) BUILT verbatim: deltas vs second opinions recorded-with-cause, never scored; "== 465"/"== 142" barred as assertion targets. BAR (b) BUILT before (c): memory/fixture_manifest.json (sha256 per anchor over derived line-set); drift test NAMES each moved line + amount; deliberate regeneration via backend/scripts_update_fixture_manifest.py citing the ruling.
- INGRESS CADENCE (ruled): external smoke (login / estimates / rederive via the PUBLIC url) runs inside every handback_green.sh after the local stamp; failure recorded as INGRESS-SMOKE-FAIL and exits loudly. Cadence held: every handback + full-suite external on demand + bar (h). Awaiting Howard's seal.
- BONI RE-READ: GO executed — rerun off the cached 11 pages, DONE (run e633ca8d). NOT applied — Howard grades on the read-back card. New read carries: gutter runs front 58 + back 58 + porch 24 = 140 (vs plane-sum 190); corner walk 10 corners / 195 LF, heights [20.5×4, 10.5×2, 9×4]; read-to-read variance vs the 8-6 read (6 corners/98) and vs Howard's evidence (8 corners/89') REPORTED, not tuned.
- STILL OWED BY AGENT: bar (e) flag census, bar (g) gate truthfulness (next session); (c) awaits Howard's typed installed lists (Letrick DELIVERED-grade + Boni; sealed key governs geometry, list is material-only evidence); (h) awaits the partner's demo script.
- Stamp: 2026-08-07 16:01 UTC · 9edeec7 · CLEAN · 1906 passed, 4 skipped · INGRESS-SMOKE-CLEAN 3 passed. Preview only.

## SESSION 2026-08-07 (part 22) — 15-DEFECT GRADE ANSWERED: RETENTION RULED IN, CONFIRMED ORDER LANDED
- ANSWERS OWED, DELIVERED FIRST: (1) DETERMINISM — 8-7 read was rerun_of 8-6 on the SAME 11 cached pages, same model (claude-opus-4-5), same transport; only display code changed between → corner drift = pure sampling noise. (2) BONI FILE — pages are 3456×2592 = ARCH-C @ exactly our 144-DPI render → came in as a PDF, but the pipeline rasterized and DISCARDED the original (0 PDFs in blob store) → probe required retention first.
- SOURCE-RETENTION RULED (Howard, verbatim in pins): "THE ORIGINAL UPLOAD IS RETAINED. Always, every door, every file type. A derived artifact never replaces its source." NAMED DEFECT: the blueprint door threw away the source at the door. Built: originals byte-identical (disk + chunked Mongo blobs — 64MB PDFs clear the 16MB doc cap), source_files/source_probe/source_text_pages on the run doc, reruns carry the source. NATIVE-TEXT PROBE (pdfium textpage census; true scan = 0 chars): per-page text stored VERBATIM as extraction ground truth. Read-Back Card opens with a SOURCE block (EN+ES). PROBE VERDICT ON BONI (Howard re-uploaded): SCAN — no text layer, 0 chars on all 11 pages; text ground truth OFF THE TABLE for Boni; Howard chasing the native export.
- TEMPERATURE=0 pinned on extraction WITH THE HARD CONDITION (ruled): cuts noise, NEVER satisfies the determinism gate, never claims correctness — "two reads agreed" and "matches the printed dimension" must never print as the same chip.
- SECOND READ (8-7 pm): ALSO REJECTED by Howard, NOT applied. Every headline number moved between runs — determinism gate fully justified (still to build). THREE GROUND TRUTHS RECEIVED (evidence, never code): (A) porch is 16'6"×6' (area 99 was READ RIGHT, dims were then invented — ruled: AN AREA DOES NOT DETERMINE A SHAPE; real short = 5 LF not 32); (B) gutter RECONCILES — runs 132.5 + damaged-piece replacement 33 ≈ 167 installed; run-inventory convention CORRECT; the 33 is a JOB EVENT; TRAP: the invented 33' porch run and the real 33' damaged piece are UNRELATED coincidences; (C) soffit-J derived 24-25 stands, list 35 = LIST-SUSPECT (~45% overage) — line not closed until the read is stable.
- BAR (c) REGRADED FOUR WAYS (ruled): MATCH · DELTA-EXPLAINED · LIST-SUSPECT · DELTA-OPEN. "List suspect, pending site walk" is a NAMED cause, never a build failure. Purity MORE binding (a wrong list is a worse tuning target).
- CONFIRMED ORDER LANDED (Howard's reorder — retention first, then):
  1. FALSE PROVENANCE (his "fix this first"): the Field Verify tab's hardcoded green VERIFIED chip retired — it asserted NOTHING. Chip now reads actual tape state: amber "NOT TAPED — AI READ" until a human tapes; green "TAPED ×n" only after (tape-check walls + dormers). fv.title/fv.tapedDims.title stop claiming taped dims (EN+ES). Verified live: chip amber on EST-886440.
  2. BACKEND MATH: gutter accessories consume the RUN INVENTORY (run count = len(runs); downspouts/elbows/hangers/end-caps/sealant divide run-sum gutter LF; basis NAMED on notes; ~1-per-30 survives only as no-inventory fallback, byte-identical). PORCH-J SPLIT: ceiling channel → SOFFIT-J line FULL PERIMETER; wall J → WALL-J line wall-abutting ONLY. _porch_geom: real dims govern (contractor Job-Info entries via _porch_dims; printed plane porch_width/depth_ft), square-assumption only as DISCLOSED MINIMUM + FLAG. Boni pins by ruling: wall-J 33→31, soffit-J 26. Manifest regenerated citing the split.
  3. BARS = CHIPS: lines carry server-computed `viz` (finish trim segments, soffit-J segments, gutter run/downspout basis); TakeoffReconCard renders ONLY line.viz — no viz, no bar; retired client formulas (full-window-perim, 2×rakes, eaves/30) banned by source pin. 637-vs-279 and 410-vs-24 dead.
  4. INGRESS SMOKE + spec-field PUT round trip (overhang_in + fascia_width_in through the public URL, throwaway estimate) — in handback cadence, 4 passed.
  5. CONSISTENCY CHECKER on the card (check_read_consistency): corner_taller_than_wall (HANDS OVER the wall table's height), corner_lf_not_sum, gable_census_mismatch, box_model (mirror + wing evidence), run_exceeds_facade; clean state NAMED. LIVE ON THE REJECTED READ: fires exactly Howard's four hand-graded contradictions (20.5 vs 18 · 188 vs 141 · 3 vs 2 gables · box model).
  6. EXTRACTION DISCIPLINE (vision path — Boni is a scan): printed window dims SACRED (printed_size/catalog_size, PRINT BOTH, never snapped 3-0x5-0→38x54); SH=single_hung end-to-end; INTERIOR DOORS dropped via exterior_evidence + FLAGGED (1→5 entry-door pollution dies); trade-spec eave_overhang_in/fascia_width_in READ or FLAGGED on the rail (printed beats form default, _overhang_src named) — never silently defaulted; corner heights READ from printed dims, NEVER ceiling-stack calculations (null + note); porch printed dims on the plane.
- New pins: test_source_retention (11), test_gutter_porch_rulings (9), test_bars_equal_chips (5), test_consistency_checker (9), test_extraction_discipline (8).
- FRESH READ TRIGGERED for Howard's grade (run db686693, rerun off cached pages, new prompt + temp=0, source carried) — NOT applied; grade-before-apply stands. Integral-J stays ON; neither 8-7 read applied to EST-886440.
...
## SESSION 2026-08-08 (part 24) — RECEIPT ANSWERED · EVIDENCE-OR-NULL + VISUAL AUDIT (ONE BUILD) · DETERMINISM GATE · TWO INVESTIGATIONS
- RECEIPT ANSWER (Howard asked which happened): the BUILD ORDER list and PURITY rider did NOT survive the fork verbatim — compressed handoff dropped the ordering ruling (evidence-or-null survived only as an implemented-item record). Consequence drawn: rulings now live in the repo — memory/REMINDERS.md gains a RULINGS REGISTER (build order, gate wording, visual-audit reqs, Mark B converter ruling, purity, EST-886440, openings convention) so a ruling survives any handoff.
- ORDER CONFIRMED AND FOLLOWED (dependency, not preference): 1) evidence-or-null + visual audit (ONE build), 2) determinism gate, 3) Mark B investigate-only, 4) Charter Oak regression note first.
- EVIDENCE-OR-NULL + VISUAL AUDIT (one schema, one renderer — coordinates never discarded):
  · DIM = {"v","page","from","loc"} or null; loc = {x_pct,y_pct,w_pct,h_pct} top-left percent box; junk locs die; a box NEVER rescues a missing quote.
  · DERIVED numbers: {"v","calc","srcs":[{page,from,loc}...]} — many highlights plus the arithmetic (the 20'-0¼" plate-stack class); one-to-one torn up by design.
  · PRECISION (design req 1): scan boxes labelled APPROXIMATE (vision pixels); native-PDF text-layer search (pdfium charbox union → percent rect) yields EXACT; a miss keeps quote-only (precision None) — never an invented box.
  · NO SOURCE FIRST-CLASS (design req 3): dropped (_nulled_no_evidence) + abstained (_dim_unread) render as loudly as highlights — VisualAuditPanel.jsx on the Read-Back Card (EN+ES), boxes overlaid on the retained page images (/api/uploads).
- DETERMINISM GATE (stability, never correctness): compute_read_stability null-aware — counts exact-match; dims tol max(0.5, 2%); evidenced per-path register compared; STABLY ABSTAINED named separately (two nulls never print as agreement on a number); value-vs-null = DISAGREEMENT. Worker persists stability vs rerun_of on the run doc; card chips "TWO READS AGREED — stability, not correctness" / per-item DISAGREE / abstained list. The word "gate" never reaches the screen (existing pin caught my first title — reworded to "Did two reads agree?").
- MARK B (investigated ONLY, converter untouched — verified SH 2-4_5-4→28x64 correct): the 28x64 came from run e633ca8d (8-7 am, pre-discipline) which transcribed B as "SH 2-4_5-4" then snapped to 38x60; the two newest reads (db686693, a6e723af) independently agree B = "SH 2-4_3-4" → 28x40. Howard reports 28x48 printed. THREE sources, THREE answers — 5-4 vs 3-4 is a one-glyph scan disagreement; needs a human eye on the schedule cell (sheets 6–7) or a tape. Report: memory/mark_b_investigation_2026_08_08.md.
- CHARTER OAK: the 3f43e6e fix HELD — live Boni estimate prints the waste step ("· ×1.00 waste (0%)"), soffit 42/42, no silent ×1.20; the RECORD (not the fix) dropped in the handoff. SIDING HIDDEN-MULTIPLIER ANSWER: NONE — the count is gross wall area ÷ 100, openings NOT deducted, ×1.0. BUT the label lied on blueprint jobs: aggregation aliased gross area into siding_with_openings_sqft so the Charter Oak note claimed the HOVER "+10%" row over a number that never received 10%. Fixed: alias dies (field None on blueprint reads), note is a callable naming the real basis; QUANTITY BYTE-IDENTICAL (purity: nothing tuned).
- New pins: test_visual_audit_2026_08_08 (12) · test_determinism_gate_2026_08_08 (12) · test_siding_basis_note_2026_08_08 (3) · evidence pins updated for the loc-bearing shape.
- Stamp: 2026-08-08 12:57 UTC · a2ce6d5 · CLEAN · 2008 passed, 5 skipped · INGRESS SMOKE 4 passed. Preview only.
- STILL OPEN: Howard's grade of the gate+audit on the NEXT rerun (first live two-read compare); bar (c) four-way fixtures (awaits typed lists); bar (e) flag census; bar (g) gate truthfulness; (h) partner script; Quote Open Receipts + cross-family fill (post-Sept).

## SESSION 2026-08-08 (part 25) — LIVE FIRE COMPLETE · PROVENANCE CORRECTION OWNED · MARK B RESOLVED FROM THE SHEET
- PROVENANCE CORRECTION (owned): the "28x48" was the app's own 4:06 PM read-back card MISATTRIBUTED to Howard. RULED: never attribute an app number to a human; a figure whose origin cannot be named has no source. Register + mark_b report corrected.
- MARK B RESOLVED (Howard read sheet 6): B = SH 3-0_4-0 · SIZE 2'-11½"×3'-11½" — ALL THREE app reads wrong; all marks are 3-0 wide; converter untouched. NEW RULINGS RECEIVED (build order item 2, HELD for after his live-fire grade): SIZE COLUMN GOVERNS (code = family label; units print ~½" under nominal — 35.5×59.5 not 36×60; moves window perimeter on every J/coil/finish-trim line); DOOR EXCLUSION SIGNALS ("HOLLOW CORE"/"Garage to House" in product-code column; exterior = E2,E3,G1,G2 only; garage doors PRINT 8'-0" — the carried 7' was a guess, "appears" = no source).
- SHEET 7 ANSWERED (agent vision transcription of RETAINED page image, named as such): SECOND FLOOR PLAN carries its OWN schedules — A×7 (3-0_5-0) + D×1 (SH 2-4_3-6 — the job's only 2-4 family, a mark NO app read ever carried). Two-sheet schedule total 16 vs reads' 21-28 — disagreement reported, awaits Howard's eye.
- LIVE FIRE (run 82bd3a5e, rerun_of a6e723af, 11 cached pages): (1) back garage wall NULL + wall_segment_undimensioned FIRED — no invented height; 10 bare corner heights dropped by construction. (2) Visual audit LIVE: 22 evidenced paths (page + verbatim quote); ONE vision box (front height, APPROXIMATE label on the elevation); dropped/unread chips first-class. (3) Gate fired on REAL drift: corners 6→10 · inside 2→6 · windows 26→19 · doors 4→5 · runs 6→5 · eaves 204→196 · rakes 132→110 · corner LF 82→188 — plain-language DISAGREE chips, zero correctness claims. NOT APPLIED to anything (grade-before-apply).
- GATE FIX FROM THE FIRE: prev run predates the evidence register → per-path compare printed 22 FALSE disagreements (schema vintage, not drift). evidenced_not_comparable now NAMED (EN+ES); per-path table stays empty rather than lying; stored stability recomputed honestly.
- BROWSER WALK caught a real defect: VisualAuditPanel import dropped by a parallel edit (part-22 lesson class) — restored, verified live with highlight box.
- BAR (c) RECONCILIATION: HELD until Monday (Howard pulling full invoice set — supplemental orders, credits, returns; installed lists not provably whole).
- Stamp: 2026-08-08 13:42 UTC · 5251593 · CLEAN · 2009 passed, 5 skipped · INGRESS SMOKE 4 passed. Preview only.
- AWAITING: Howard's grade of the live fire (3 items). THEN build order: 2) size-column ruling + door exclusion signals, 3) flag census (bar e), 4) gate truthfulness (bar g), 5) reconciliation (Monday).

## SESSION 2026-08-08 (part 26) — FIRE GRADED (PASS/PASS/PARTIAL) · OCR-FOR-COORDINATES · SIZE COLUMN GOVERNS · DOOR EXCLUSIONS
- FIRE GRADES: item 1 PASS ("empty and honest beats populated and invented — do not soften it"), item 3 PASS (the DISAGREE content is the news: no read is trustworthy yet and the app said so plainly), item 2 PARTIAL (1 box in 22) → this build.
- OCR-FOR-COORDINATES (ruled): local rapidocr (pip-only, deploy-safe) over retained page rasters; text runs indexed with boxes; model's VERBATIM quote matched under alphanumeric normalisation (58'-0" == OCR's 58-0°). HARD SEPARATION pinned: OCR SUPPLIES LOCATION, NEVER VALUE (touches loc/precision only). Precision ladder: exact > ocr (blue "machine text-read" label) > approximate > quote-only. FREE SECOND READ: unfindable quotes → ocr_quote_miss rail flag (two reads of the same pixels disagree; neither wins). RETRO PASS on run 82bd3a5e: 10/22 boxed (was 1/22); 11 misses — mostly 20'-0" elevation quotes: rotated print OCR missed OR invented quotes; flag says exactly that. KNOWN LOCATOR GAP: vertical/rotated dimension strings.
- SIZE COLUMN GOVERNS (ruled off the printed Boni schedules): prompt rewritten — SIZE column IS the dimension; product code = FAMILY LABEL (new product_code field, parsed only when no size column); units print ~½" under nominal. BONI TRUTH (evidence, not targets): 16 windows (A 9=2+7 · B 1 · C 5 · D 1); window perimeter 252.0 LF vs carried 334.5–447.3 (33–77% over on the J/coil/finish-trim driver). MARK-SUMMING ACROSS SHEETS: one row per mark, qty summed, schedule_pages listed; mark_size_conflict LOUD flag pins the exact Mark-B merge signature (one mark, two printed sizes — D's dims on B's letter). Parser learned feet-inch-fractions (2'-11 1/2" x 4'-11 1/2" → 35.5×59.5) with all pre-ruling formats regression-pinned.
- DOOR EXCLUSIONS + PRINT HEIGHTS (ruled): product-code column is THE exclusion signal (HOLLOW CORE / H DWL CORE / Garage to House = interior, never inferred from mark prefix; Boni exterior = E2,E3,G1,G2 only). Doors gain printed_size verbatim; "appears" = no source → null+flag; door_size_parse_mismatch LOUD flag pins the live failure (printed 16'-0"×8'-0", carried 192×84).
- LANDMINE FOUND: EN dictionary strings containing literal "es: {" truncate the Spanish-parity block splitter — reworded, recorded in REMINDERS.
- New pins: test_ocr_locate (7) · test_size_column_governs (9). Stamp: 2026-08-08 15:49 UTC · ba2ce08 · CLEAN · 2025 passed, 5 skipped · INGRESS 4 passed.
- NEXT (Howard's order): 4) FLAG CENSUS (bar e), 5) GATE TRUTHFULNESS (bar g), 6) reconciliation HELD until Monday's invoice set (complete as to ALSIDE, not the JOB). A fresh rerun would exercise the new schedule prompt — awaits Howard's GO (costs a read).

## SESSION 2026-08-08 (part 27) — THE ELEVEN RESOLVED: FABRICATED QUOTES · COMPUTED-IS-DERIVED RULED · SCHEDULE RERUN REPORTED
- ROTATED LOCATOR built (90°CCW+CW passes, boxes mapped back upright; miss records attest rotations_checked). VERDICT, deterministic: ZERO of the eleven locate in any orientation, while the printed COMPONENTS of the stack DO locate rotated on the same sheet (9'-11 1/8" found vertical on sheet 1). The 20'-0" totals were FABRICATED QUOTES — evidence-or-null was evidence-or-fiction for computed totals; the free second read surfaced it. Sheet 6's 39'-0": not found while 58'/24' locate — consistent-with-computed, reported not crowned.
- RULED INTO THE CONTRACT: IF A VALUE IS COMPUTED, IT IS DERIVED — {v,calc,srcs[]} per component; "A COMPUTED NUMBER WEARING A QUOTE IS A LIE WITH A CITATION"; quotes must be PRINTED AS SUCH.
- SCHEDULE RERUN 840b34e8 (fresh read on the new contract) — REPORTED, NOTHING TUNED:
  · LANDED: size column governs (35.5×59.5-class values, product_code separate); B = SH 3-0_4-0/35.5×47.5 exactly as printed; garage doors 8'-0" from print; A sums across sheets [6,7]; derived form used on front height (calc + 2 srcs); gate's first true post-register compare: evidenced 22/22 agreed.
  · DID NOT LAND ON 16: windows total 23 (12/1/9/1 vs printed 9/1/5/1) — the COUNT column is still not read (symbol-counting suspected); D drifted to a THIRD variant (SH 2-4_3-4/3'-3½"); an E1 exterior entry the sheet does not hold; G2 9'-2" vs printed 9'-0"; several heights still single-quote 20'-0" (locator flags them); derived srcs quote 9'-1 1/8" where the sheet prints 9'-11 1/8".
  · Candidate ruling for Howard: "the COUNT column governs counts" (same shape as size-column). NOT built — his call.
- SILENT-TRUNCATION SCOPING answered in the report to Howard (single detector: no; single accounting convention + seam registry + census pin: yes, buildable; external compressions like the handoff summarizer are out of reach except by the repo-side rulings register).
- New pins: test_rotated_locator (3). Stamp: 2026-08-08 21:38 UTC · 1974a85 · CLEAN · 2028 passed, 5 skipped · INGRESS 4 passed.
- NEXT: Howard's response on the count-column candidate ruling + truncation-detector scoping → then FLAG CENSUS (bar e), GATE TRUTHFULNESS (bar g); reconciliation HELD until Monday.

## SESSION 2026-08-09 (part 28) — COUNT COLUMN ENFORCED · MARKS FACE THE LOCATOR · SEAM ACCOUNTING (Howard's confirmed order, send c)
- ORDER CONFIRMED 1→2→3, correction carried: BOTH HALVES — never count symbols on floor plans; DO sum the printed COUNT column ACROSS sheets for the same mark (A: 2 on sheet 6 + 7 on sheet 7 = NINE; a per-sheet mark row is not a distinct mark). STOP AFTER ITEM 3 (his option b) — flag census (bar e) + gate truthfulness (bar g) WAIT for his card walk.
- (1) COUNT COLUMN GOVERNS — ENFORCED AT THE SEAM (_enforce_count_column in the worker pipeline; the prompt rule alone had left the rerun at 23 vs printed 16): per-sheet mark rows MERGE (only when count cells exist AND printed sizes agree — Mark-B one-mark-two-sizes rows stay separate for a human); carried qty ≠ cell sum → qty REWRITTEN from the cells + LOUD count_column_governed; a mark with no printed count CONTRIBUTES NOTHING (qty 0, aggregation skip-proofed against max(1,..) resurrection) + LOUD count_cells_unread; same-sheet conflicting cells LOUD; no-cells-anywhere → warn count_column_absent, legacy qtys untouched; elevation-split rows without cells untouched. Doors gained count_by_page/schedule_pages and ride the same enforcement. EN+ES rail strings.
- (2) MARKS FACE THE LOCATOR TOO (_ocr_verify_marks): every schedule row's quotes (mark, printed_size, product_code) searched on the row's own sheets, upright + both rotations. NO quote locates → row DROPPED as fabrication (the E1 class), accounted, LOUD mark_not_located. Mark locates but printed size doesn't → quote KILLED: dims null, claim preserved in printed_size_not_located, parse never reaches the takeoff (the G2 9'-2" class), LOUD mark_quote_miss. Mark matching exact-or-prefix, NEVER containment (E1 can't hide inside E2…). OCR engine failure is NEVER evidence — rows stand. Hard separation held: OCR supplies existence, never value.
- (3) SEAM ACCOUNTING (seam_accounting.py): registry + ledger — every removing layer records what it removed (interior doors, evidence nulls, count rewrites, unread counts, mark merges, fabricated drops, killed size quotes, THE CUT's lp_smart rows); unregistered seam RAISES; AST DETECTOR pin scans ai_blueprint.py for filtering comprehensions over data collections — a new unreviewed filter FAILS THE BUILD BY NAME (the qty-0 / "es: {" silent-truncation class). Ledger rides the readback ("seams") and the rail, EN+ES.
- PROFILE MECHANISM REPORT (Howard's Addition 1 — report only, NO fix): the swap chips live in PerElevationBreakdownCard, DOUBLE-gated: (a) `return null` when measurements._per_elevation_breakdown is empty — only the photo aggregator fills it; (b) the card mounts ONLY inside the two run dialogs (AIMeasureButton / BlueprintMeasureButton), never on the estimate page. The banner he saw is a third run-gated surface (ElevationSheetsPanel). SURVEY: manual accent injection (add-a-profile-Claude-missed) lives in the SAME card → identically unreachable on blueprint-only estimates — the predicted "something else" — REPORTED, NOT FIXED, awaits ruling. Photo fill-ins are gated by HIS OWN ruling (photo door only) — deliberate, not a defect. Color/tier pickers + trade-spec fields are estimate-page surfaces, not gated. LAST SESSION'S SidingProfileChip (estimate-page mount, both rulings: every door + DEFAULTED PRINTS AS DEFAULTED) verified live by browser walk on "boni 8-9-26 10am": picker present, 25 priced options, zero measurement runs.
- PURITY held: 2/7/9/16/23, E1, G2, 9'-2" are evidence and test fixtures — never constants, defaults, or targets. Nothing tuned; a fresh rerun (costs a read) would show where the enforced pipeline lands — awaits Howard's GO.
- New pins: test_count_column_governs_2026_08_09 (13) · test_mark_locator_2026_08_09 (10) · test_seam_accounting_2026_08_09 (9, incl. the AST detector + allowlist-liveness pin).
- Stamp verbatim: `2026-08-09 15:18 UTC · 9995a29 · CLEAN · [tests] · 2067 passed, 5 skipped, 6 warnings in 123.70s (0:02:03)` · INGRESS SMOKE: 4 passed.
- NEXT (held for Howard's card walk per his stop-after-3): 4) FLAG CENSUS (bar e) 5) GATE TRUTHFULNESS (bar g) · reconciliation (bar c) HELD until Monday's invoices · accent-injection un-gating awaits ruling · EST-886440 untouched, Integral-J ON.

## SESSION 2026-08-09 (part 29) — STALE-PAGE BANNER · ACCENT UNGATED · PROMPT-ONLY AUDIT · LIVE RERUN 76203e6a REPORTED
- VINYL non-defect closed by Howard (stale page, hard refresh resolved; swap verified E2E with colours following). RULED + BUILT: STALE PAGE DETECTION — GET /api/version (git commit, once at import; stamp fallback); BuildMismatchBanner on every route remembers the loaded version, re-checks on focus + 5 min, mismatch prints the CLIENT/SERVER PAIR + refresh prompt EN+ES; network failure NEVER fires it; seam REGISTERED (client_build_stale).
- RULED + BUILT: ACCENT INJECTION UNGATED ON EVERY DOOR — PerElevationBreakdownCard now mounts on the estimate page (vinyl tab) wired to est.hover_measurements + /measure/map + save; empty breakdown renders a NAMED state (per-elev-empty), never a silent null. Photo fill-ins stay photo-gated BY RULING. Browser-walked live on boni 8-9-26 10am (4 elevations, chips, +Add Accent).
- PROMPT-ONLY AUDIT delivered (memory/prompt_only_rulings_2026_08_09.md): headline — evidence-or-null covers a MINORITY of numerics; roof planes/gutter runs/appendages/gable rises/opening dims/accents/avg height all bare numbers; door-exclusion SIGNAL unverified behind the model's label; SH/DH retype unchecked; profile callouts unverified. Candidate seams a–d listed, NOT built.
- LIVE RERUN 76203e6a (rerun_of 840b34e8) — REPORTED, NOTHING TUNED:
  · WINDOWS 20 vs printed 16: A=9 BY THE COLUMN (2+7 across sheets) ✓, B=1 ✓, D=1 ✓; C=9 where the sheet prints 5 — the model wrote 9 INTO the count cell; enforcement makes the column govern, it cannot make the cell read right. 23→20, error localized to ONE named cell. OCR shows 3× 'SH3056' on sheet 6 while the read gives C A's code (SH 3-0_5-0) — C may be the 3-0_5-6 family, Howard's eye decides.
  · E1: machine verdict — "E1" prints NOWHERE on sheet 6 in any orientation (LOUD mark_quote_miss doors:E1.id); E2 DOES print and is ABSENT from the read (transcription suspected). Row NOT dropped: drop requires ALL quotes to miss and E1's 2-char size quote "3'-0""→"30" trivially exact-matched a dimension run. FINDING FOR RULING: quotes ≤2 chars should carry no survival veto (one-line fix, not built — changes what evidence saves a row).
  · G2 9'-2": KILLED as designed — dims null, claim preserved, ledger doors:G2; diagnostic found '90X80' printing right there (9'-0"×8'-0"). Fabricated quote killed inches from the true print.
  · D: drift named by MACHINE twice — size quote AND code 'SH 2-4_3-4' both unfindable on page 7, both loud, dims nulled.
  · A/B/C size quotes (2'-11 1/2"-class): none located, dims nulled + loud. TWO READINGS reported: model added the ½ fractions, OR OCR fragments fraction glyphs (nearest sheet-6 runs '211411','2113119'). Machine can't distinguish — Howard's eye on the SIZE column decides. DISCLOSED: run carries null window dims (abstention, not invention). INSTRUMENT LIMIT named: multi-token quotes must sit inside ONE OCR run — biases long fraction-heavy strings toward kills; candidate fix = adjacent-run joining before verdicts (not built).
  · Seam ledger LIVE on the card: mark_size_quotes_nulled removed 5. Zero count rewrites needed (model's carried qtys matched its cells this run). Stability vs 840b34e8: corners 12→8/8→4, windows 23→20, doors 5→4, runs 5→6 — DISAGREE chips fire.
- New pins: test_build_version_2026_08_09 (9). Stamps: `2026-08-09 17:16 UTC · 813c1d7 · CLEAN · 2076 passed, 5 skipped` · INGRESS 4 passed.
- AWAITING HOWARD: card walk (bar h) → then flag census (e) + gate truthfulness (g); rulings pending on: ≤2-char quote veto, adjacent-run joining, candidate seams a–d from the audit. Bar (c) HELD until Monday's invoices. EST-886440 untouched, Integral-J ON.

## SESSION 2026-08-09 (part 30) — BUILD ORDER 1-5 DELIVERED (send 3): OMISSION · JOINING+SKELETON · VETO · BARE-NUMBER EVIDENCE · DOOR SIGNAL
- (1) OMISSION CHECK: schedule-code + door-mark tokens inside the table region (anchored by read rows, padded below — omitted rows sit under the read ones) with no read counterpart → LOUD schedule_row_omitted. One-glyph drift (SH340~SH3040) never flags; door candidates restricted to the read's own door letters (NAMED LIMIT: an entire unread letter family is beyond the instrument); <2 anchors = undecidable, skipped. LIVE on Boni rasters: SH3056 (C's real family), E2 (the absent door), SH2436 (D's real family) — ZERO noise.
- (2) ADJACENT-RUN JOINING in dim locator + mark verifier (all orientations) + FRACTION SKELETON: OCR cannot read stacked ½ glyphs at all — size quotes also try fraction-stripped / x-less skeletons; skeleton locate is NAMED (info skeleton_match) and still kills a wrong quote. LIVE RE-KILL vs run 76203e6a: A DIMS LIVE, B DIMS LIVE (both skeleton — the prints Howard verified), C KILLED (5'-0 vs printed 5'-5), D KILLED (size+code drift), G2 KILLED (9'-2 vs 9'-0), E1 DROPPED under the veto. Instrument now passes good data AND kills bad data.
- (3) SHORT-QUOTE VETO: ≤2-char quotes carry NO survival weight; the mark keeps its weight. PROXIMITY RULE SCOPED not built: the omission region machinery (anchor bbox) can gate survival matches to the table region; needs per-match box plumbing (upright only — rotated matches have no comparable frame) + anchor-gated fallback. ~1 item build; awaits ruling.
- (4) BARE-NUMBER EVIDENCE: roof-plane eave_lf/rake_lf + gutter-run lf are DIM|null in BOTH prompts; enforcement flattens quotes + NULLS bare numbers (accounted); _dim_v lets roof-pass merge compare magnitudes with evidence intact (surgical rake merge carries the DIM whole).
- (5) DOOR SIGNAL BY MACHINE: schedule line carrying mark + interior marker (HOLLOW CORE/H DWL CORE/GARAGE TO HOUSE) drops the row regardless of the model's label; seam interior_signal_dropped, loud rail EN+ES.
- Seam detector caught its own widened drop filter (allowlist updated) — the detector works.
- Pins: test_omission_and_veto (14) + test_bare_number_evidence (9). Stamp verbatim: `2026-08-09 18:32 UTC · 50e08f7 · CLEAN · [tests] · 2098 passed, 5 skipped, 6 warnings in 133.01s (0:02:13)` · INGRESS 4.
- NOTE: these instruments are IN the worker pipeline — the NEXT live rerun (costs a read, Howard's GO) exercises omission/skeleton/veto/door-signal + evidenced planes/runs end-to-end on a fresh read. Run 76203e6a (his card walk) predates them except via the offline re-kill.
- AWAITING: Howard's card walk (bar h) → flag census (e) + gate truthfulness (g); rulings pending: proximity rule build, omission reach-further options (profile-callout census on elevations; planes/runs have no text marks — geometric census needed), next rerun GO. Bar (c) HELD until Monday. EST-886440 untouched, Integral-J ON.

## SESSION 2026-08-09 (part 31) — SEND 4 DELIVERED: FRESH RERUN a4cbce91 · MARK-MERGE ONE-DEFECT FIX · PROXIMITY · CALLOUT CENSUS
- FRESH INSTRUMENTED RERUN a4cbce91 (rerun_of 76203e6a) — HOWARD WALKS THIS CARD (bar h): WINDOWS 16 BY THE COLUMN (9/1/5/1); E1 dropped (fabrication); E3 missing from the read and CAUGHT by omission (both directions instrumented within one day); SH3056/SH2436 flagged omitted; A dims live; B killed = NAMED instrument miss (print real per Howard, sheet 6 fragments defeat locator, quote preserved loud); C killed (A's size copied by merge); D killed (3rd height variant 3'-3½); G2 killed; planes+gutters ALL evidenced, zero bare numbers; seam ledger whole; corners still drift 6/2 (unevidenced family — DISAGREE chips fire).
- MARK-MERGE MECHANISM (evidenced): model collapses sibling rows sharing a long code prefix, copying trailing cells — row identity not preserved. ONE FIX: ROW IDENTITY prompt rule + mark_merge_suspected LOUD seam (sharers named + likely unread sibling via 1-sub/1-del vs omissions: "A + C share SH3050 while SH3056 prints unread") + suspicion revokes matching leniency (C's copied quote dies; unsuspected A locates).
- PROXIMITY RULE built (class over instance): matches must sit in the anchor-derived table region (upright-only NAMED blind spot; <2 anchors → page-wide fallback); pool = region-INTERSECTING runs; in-region containment cap relaxed (region is the constraint). E1-class out-of-region match no longer saves a row.
- CALLOUT CENSUS built: SHAKE/BATTEN/DUTCHLAP/SCALLOP on identified elevation sheets vs read families → loud callout_omitted. Boni: TRUE NEGATIVE (elevations print only VINYL). Sits beside the ungated accent injection + gables banner.
- GEOMETRIC OMISSION GAP logged (memory/prompt_only_rulings_2026_08_09.md) — named, not ordered.
- Pins: test_mark_merge_proximity (9). Stamp verbatim: `2026-08-09 20:24 UTC · e55a742 · CLEAN · [tests] · 2107 passed, 5 skipped, 6 warnings in 129.85s (0:02:09)` · INGRESS 4.
- AWAITING: Howard's walk of card a4cbce91 → then FLAG CENSUS (e) + GATE TRUTHFULNESS (g). Bar (c) HELD until Monday. NOTE: run a4cbce91's STORED registers predate items 2-4 (merge/proximity/census verdicts above are the offline pass); the NEXT rerun stores them natively. EST-886440 untouched, Integral-J ON.

## SESSION 2026-08-09 (part 32) — SEND 5: MARKED-UP TAKEOFF ELEVATIONS SCOPED (NOT BUILT — gated behind card walk + e/g)
- Howard ordered Approach (A): synthetic elevation rendered FROM the blueprint model, coloured takeoff (AREA fills + LINEAR edges, qty beside each swatch), honesty hatching ("NEEDS YOUR TAPE") on null/derived walls; zone model as a FIRST-CLASS reusable layer; (B) paint-the-real-sheet DEFERRED not dropped.
- SCOPE DELIVERED (memory/takeoff_elevations_scope_2026_08_09.md): Q1 — photo EL renderer IS reusable: SheetSvg (frontend, pure SVG from a JSON contract) needs zero Phase-1 changes; the coupling is one query (endpoint hard-binds ai_measure_runs) → build a blueprint contract builder. Q2 — missing from blueprint read: opening x-positions (don't exist; Phase 1 = schematic spacing, LABELED), wall attribution unverified (prompt-only family), per-edge corner assignment (derivable from wall_segments), line binding for live quantities. Q3 — ≈3 sessions (contract 1 · zone model + legend 1.5 · 4-elevation instrument card 0.5).
- REMINDER SET (Howard: "remind me later if needed"): previous send-4 report (rerun a4cbce91 = his walk card; corners still unevidenced; B's kill = named instrument miss) stands in PRD part 31.
- QUEUE UNCHANGED: Howard walks card a4cbce91 → FLAG CENSUS (e) + GATE TRUTHFULNESS (g) → THEN takeoff elevations Phase 1-3. Bar (c) HELD until Monday's invoices. EST-886440 untouched, Integral-J ON.

## SESSION 2026-08-09 (part 33) — SEND 6: RUN IDENTITY ANSWERED + PUT ON THE CARD
- ANSWER: run a4cbce91 = EST-886440 "Boni 8-6-26 4pm", fired 2026-08-09 20:10 UTC (chain 840b34e8 → 76203e6a 17:18 → a4cbce91 20:10). RIDER HELD: nothing applied; estimate doc untouched (updated_at 8-7 20:29 predates both reruns); grade-before-apply stands — runs on EST-886440 are the established GRADING pattern, never applied.
- RULED + BUILT: run identity ON THE CARD — readback header prints `run <id8> · <fired-at> UTC` (status payload attaches readback.run), EN+ES, testid bp-rb-run-identity. Going forward all walk references = ESTIMATE NUMBER + RUN TIMESTAMP.
- Stamp verbatim: `2026-08-09 23:15 UTC · b68724a · CLEAN · 2110 passed, 5 skipped, 6 warnings in 136.14s (0:02:16)` · INGRESS 4.
- QUEUE: Howard walks EST-886440 run of 20:10 UTC → FLAG CENSUS (e) + GATE TRUTHFULNESS (g) → takeoff elevations Phase 1 (scope filed). Bar (c) HELD until Monday.

## SESSION 2026-08-09/10 (part 34) — SEND 7: REGISTER AUDIT DELIVERED + ALL FIVE ORDERS LANDED
- REGISTER AUDIT (report, bidirectional): ① REGISTER-SAYS-SEAM/CODE-DOES-NOT — census accent leg dead (`w.get("accents")` vs schema `accent_profiles`); page cap = unregistered removing seam; stale register lines. ② All other SEAM claims verified in executing code. ③ CODE-ENFORCES/REGISTER-SILENT — roof second pass (unregistered merge seam), image-cap slice, `_del1` tolerance, census keyword coverage.
- ROOF PASS MECHANISM (memory/roof_pass_mechanism_2026_08_09.md): fires on garage evidence only, ≤5 sheets. CONFIRMED DEFECT: roof-pass schema carried BARE corner heights vs main schema DIMs; merge runs pre-enforcement → quoted heights replaced by bare numbers → nulled by the evidence gate (evidence + value destroyed — why corners never stabilised). Corner acceptance is a MAX-WINS RATCHET (higher count always wins — the 6→10→12→8→6 signature). Pitch overwrite cannot flip gable assignment (not the EST-040221 flip by itself). Garage-plane append = the gutter double-count mechanism.
- BUILT (all handback-green): (1) roof_pass_overwrite seam registered + ledgered old→new + NEVER-TOUCH RULE (evidenced never replaced by unevidenced; refusals on rail `roof_pass_rejected`); roof-pass schema now demands DIM heights. (2) Consumer-key detector widened to every model-schema consumer (test_schema_consumer_keys); accents key FIXED; detector's first run caught vent_count/shutter_count consumed-but-never-requested (Q7) — schema now asks. (3) pages_truncated seam: ledger + LOUD card flag "N → M"; defaults 12→20 (hard 28); 24MB total recompress guard; AST detector widened to slices/min-len (named limit: len-to-var indirection). (4) Named limits registered (_del1, census keywords, roof pass ≤5 sheets). (5) Stale register lines corrected (count-column landed; silent-truncation built); CALLOUT FABRICATION GAP logged beside geometric gap. EST-886440 UNTOUCHABLE guard: 423 on every write route (backend/untouchable.py), completeness-pinned.
- OPEN FOR HOWARD'S RULING: corner max-wins acceptance (candidate: agreement-or-flag) · rakes_lf larger-wins race (candidate: plane sum governs when planes exist).
...
## SESSION 2026-08-10 (part 36) — SEND 8-2: WING GUARD + PORCH GEOMETRY + FIELD NAMES + BARS (e)(g) CLOSED + PHASE 1 RENDERING
- WING FLAG GUARD (ruled): `opposing_walls_disagree` flags loud with BOTH widths; box-model rect takes the SHORTER of a disagreeing pair (max() could suppress the wing detector — a missing body costs the job). Pinned on the Boni shape.
- PORCH WALL-SIDE FROM GEOMETRY (ruled): porch plane's eave names the wall run (±1'); undeterminable → flag + DISCLOSED MINIMUM; longer-side assumption dead (right on Boni 16'6"×6' by luck only).
- FIELD NAMING (ruled): `vent_unit_count` / `shutter_panel_count` — counting unit stated in the prompt; downstream confirmed (vents qty = units; shutters pairs = ceil(panels/2)).
- BAR (e) CLOSED: all 48 rail/consistency codes carry EN+ES text + firing pins (census found and fired `no_pitch`, `read_notes`). BAR (g) CLOSED: every gate code reachable (census caught `facade_scope_composed` + `pending_price` unpinned — now pinned); empty-means-empty + fire-from-lines pinned. Live demo: DEMO-LETRICK EMPTY (21 priced lines), EST-910869 EMPTY (0 lines).
- PHASE 1 STARTED AND RENDERING: `routes/blueprint_elevation.py` → photo-door sheet contract from the blueprint model; SheetSvg UNCHANGED. Honesty on the sheet: quoted dims tag ✓ with verbatim quote+page; unquoted ⚠; unread heights hatch NEEDS YOUR TAPE; positions SCHEMATIC and say so; merge-suspects flagged; stepped walls name printed segments. Page `/estimate/:id/blueprint-elevation/:which` + entry links on the Blueprint card. Verified live on Boni run a4cbce91 (EL-1: 58'-0" quoted p.6, 13 openings, roofline bound). Remaining phases: 2 (verification loop), 3 (4-elevation card), stepped-frame drawing.
- Stamp verbatim: `2026-08-10 18:13 UTC · 9242eda · CLEAN · 2167 passed, 5 skipped, 6 warnings in 123.86s (0:02:03)` · INGRESS 4.
- QUEUE: Phase 1 walk by Howard → Phase 2/3 on his order → RECONCILIATION (c) HELD until invoices. EST-040221 flip still unattributed (instrument now live). EST-886440 guarded, Integral-J ON.

## SESSION 2026-08-11 (part 37) — TTL INCIDENT #3 CLOSED CLASS-WIDE: PHASE 1 (TTL RAISE) + PHASE 2 (STORAGE AUDIT) + PHASE 3 (FULL TIER A) + ELEVATION MECHANISM REPORT
- PHASE 1: `ai_blueprint_runs` TTL raised 24h → 30 DAYS (collMod in place, live-verified 2592000). The July "heavier image payloads" rationale CHECKED AND DISPROVEN: run docs carry paths + JSON only (~70KB sibling avg); disproof written into startup.py. UI boundary text + pins updated; stale "24h" string BANNED by pin. Boni unblocked; Howard re-read (run 80c10620).
- PHASE 2: STORAGE-LAYER AUDIT delivered (memory/storage_layer_audit_2026-08-11.md), report-only. Live-DB walk of all 22 collections: 5 TTL indexes, ZERO ledgered; hover_import_runs @24h = same anti-pattern (A3); hover_page_cache ORPHANED 1h index (A5 trap); no blueprint dead-worker sweep (B1 compound failure); boot-time vero purge unledgered (C1); no capped collections; estimates/fixture_runs/upload_blobs clean.
- PHASE 3 (all 7 orders, suite-green): (1) hover_page_cache orphaned index DROPPED at boot, census-pinned gone. (2) ARCHIVE-ON-VIEW across ALL run types (blueprint/measure/hover status + latest-for endpoints; terminal runs only; probes excluded) + PROTECTED-estimate auto-archive at worker completion (3 workers, blueprint error path included) + boot backfill of every protected estimate's runs. (3) Blueprint DEAD-WORKER BOOT SWEEP: flips corpse to class-5 error AND ARCHIVES it — crash evidence outlives the TTL. (4) EXPIRY ON THE CARD: every run response carries reaped_at/archived; BlueprintReadBackCard prints "reaped at {when} unless applied/archived" / "archived — never reaped" EN+ES (verified live on Boni: "run 80c10620 … archived — never reaped"). (5) FULL TIER A LEDGERED: SEAM_REGISTRY += ttl_reaper + vero_obsolete_boot_purge; TTL_REAPER_REGISTRY names all 4 windows+why. (6) INDEX CENSUS (test_ttl_index_census): walks the LIVE DB — build fails on unregistered expiring index, stale registry entry, window mismatch, capped collection, forbidden-collection TTL. (7) vero boot purge ledgered via account(). Read-path pins updated: archive-on-view is the ONE permitted read-path write (a copy, never a mutation).
- ELEVATION MECHANISMS (report only, memory/elevation_mechanisms_2026-08-11.md), run 80c10620: (1) GARAGE STEP = RENDERER — model carries front segments (34'@20' + 24'@10'6"); renderer hands SheetSvg `segments: None` by construction (Phase 2 maps them). (2) FRONT GABLE = MODEL — plane-level read carries 4 gable ends (garage/bonus plane has 2) but wall-level attribution has only 2 (left/right); 4-vs-2 contradiction UNCENSUSED — candidate instrument: gable-ends census. (3) PORCH = RENDERER — model carries 16'6"×6' 99ft² plane; Phase 1 never reads porch. (4) 20-vs-16 WINDOWS = READ DISAGREEMENT — this run's mark C claims count cell 9 (p.6); ground truth/prior chain read 5; count-column enforcement had nothing to correct (qty agreed with its own cell); AND stability=None — the determinism gate was BLINDED because the reaped chain left no prior run. The incident cost a real detection. (5) G2/D null×null = locator killed REAL quotes (as ruled; model note reads G2 9'-2"×8'-0" vs Howard's 9'-0" — disagreement reported, not tuned). (6) LEFT AREA = RENDERER defect — rectangle-only 780 ft² silently excludes the 11'4½" gable triangle (~222 ft²).
- Stamp verbatim: `2026-08-11 19:22 UTC · c2b9ddb · CLEAN · 2182 passed, 5 skipped, 6 warnings in 127.03s (0:02:07)` · INGRESS 4.
- QUEUE: Howard rules on mechanism report → ELEVATION PHASE 2 (segments drawn, porch, gable-ends census, area honesty) → RECONCILIATION (c) HELD until invoices. EST-886440 guarded, Integral-J ON. Consider `protected: true` on EST-886440 so the boot backfill archives every future run automatically.

## SESSION 2026-08-13 (part 88) — SEND-11: DEMOTE-ALL + MISREAD TIER + OCR-COVERAGE WEIGHTING + MATERIAL ZONE LAYER COST
- ITEM 1 LANDED (amends send-10 item 1): `_one_source_one_path_guard` now DEMOTES ALL consumers when a quote is shared. Send-10 kept the alphabetically-first — a coin flip; send-11 killed it. Both Boni side walls sharing the 39'-0" quote lose evidence; neither may claim AI-READ. Shared-source ledger records `kept=None` and every consumer under `demoted`. Two send-10 tests amended to assert demote-all.
- ITEM 2 LANDED: MISREAD tier between FABRICATED and UNVERIFIED. When a quote is absent from a page but a real OCR run sits ONE character-edit away (`_del1`/`_sub1`, same helper as the SH340↔SH3040 glyph-drop class), the miss lands on `_dim_misread` with `misread_of` naming the real string. Value still nulls for money — a misread cannot feed money any more than a fabrication — but the diagnosis is "transcription typo" not "invention". Rail code `dims_misread` loud, EN+ES dictionary entries added. Boni's 32'-5 1/2"→33'-5 1/2" case is the pin.
- ITEM 3 LANDED: OCR-coverage weighting on FABRICATED. Every page's OCR run count (upright + rotated + joined norms) rides on the miss record as `page_ocr_chars`; on classification, fabricated records stamp `evidence_strength = "strong"` (>300 char-normalised tokens) or `"weak"` (≤300). Rail badges each path `[strong]`/`[weak]`. A poorly-read stacked-fraction page will not be branded as a fabrication with high confidence.
- SEAM REGISTRY: `dims_misread` added; `dims_demoted_quote_shared` copy updated to name send-11 demote-all. INTERNAL_KEYS registered: `evidence_strength`, `misread_of`, `quotes`.
- TEST PINS: `test_send11_demote_all_and_misread_tier_2026_08_13.py` — 14 cases across all three items. Full suite: **2320 passed, 5 skipped** (up from 2306). Two live-DB flakes in `test_guard_extension_2026_08_11.py` (`test_tape_check_write_lands_in_the_ledger`, `test_profile_annotations_write_lands_in_the_ledger`) trip on the endpoint's `.limit(200)` — pre-existing at commit 8a986c7 (send-10 CLEAN), unrelated to send-11. Reported, not touched — awaits the ruling.
- MATERIAL ZONE LAYER COSTED (report only, `memory/material_zone_layer_cost_2026-08-13.md`): PDF-overlay editable form has NEVER been costed before. **MUV = ~4 sessions** to land "correct a wall on the real drawing and see the quantity move": pdf_overlay_polygons collection + CRUD + polygon→sqft with 3/16"=1'-0" default + rebuild survival via existing `qty_src=="human"` guard in hover.py + a new `PdfOverlayEditor.jsx` page reusing the existing `image_payloads` rasterisation. Simplified synthetic sheets kept as secondary summary. MUV excludes snapping, PDF export, multi-user editing. Awaits ruling before start.
- QUEUE (restated from the register, not from memory): 1. DEMOTE-ALL ✓ 2. MISREAD ✓ 3. OCR-COVERAGE ✓ 4. P0 chips + 3 misfiled surfaces onto JobInfoPanel · 5. MATERIAL ZONE LAYER (PDF-overlay editable, MUV first) — awaiting ruling on cost · 6. Verdict-and-Triage card · 7. Ledger UI on Blueprint card · 8. EST-040221 orientation flip · 9. Reconciliation (bar c) HELD until invoices.
- PURITY STILL HELD (do not prompt harder): gable over-count 6 vs 4, porch 195 vs 99, corners 12/8 vs 8/4. Nothing applies to EST-886440. Integral-J stays ON.

## SESSION 2026-08-13 (part 89) — SEND-11 PRO-QUOTES REPLY: LEDGER TRUNCATION FIXED + TWO REPORTS FOR RULING
- ITEM 1 (BLOCKED HANDBACK) LANDED: `/api/estimates/{eid}/protected-ledger` `.limit(200)` was the third removal shape (after filtering comprehensions and slice/min-caps). It sat INSIDE the instrument built to make every human write to a sealed estimate visible. Endpoint now returns `total` (via `count_documents`), `showing`, `page`, `page_size`, `truncated`, `truncation_notice` ("showing N of M — page X of Y"). Accepts `?page=` / `?page_size=` (hard cap 1000). Seam `protected_ledger_paginated` registered. The two send-10-era tests amended to observe `total` (honest) instead of `len(entries)` (silently capped). New test pin `test_send11b_protected_ledger_truncation_honest_2026_08_13.py` — 8 cases including a synchronous pymongo negative control that fires the moment anyone reintroduces a silent `.limit()`.
- HANDBACK STAMP CLEAN: `RECORDED: 2026-08-13 02:23 UTC · 5d51fa5 · CLEAN · RESULT: 2322 passed, 10 skipped, 6 warnings in 144.49s (0:02:24) · INGRESS SMOKE: 4 passed`.
- REPORT ONLY, NO BUILD: **Detector inversion proposal** (`memory/detector_inversion_proposal_2026-08-13.md`). Feasibility YES, worth it YES, ~2.5 sessions to generalise the boundary-envelope pattern the ledger fix just proved. The AST detector is at 3 shapes and losing; a wholeness contract at the BOUNDARY (every crossing declares `total` / `emitted` / `truncated` / `filter_names`) turns the catalog problem into a declaration problem — same move `SEAM_REGISTRY` and `test_normalizer_registry` already made for their respective classes. Awaits ruling.
- REPORT ONLY, NO BUILD: **Material Zone Layer LINEAR-EDGES + legend increment cost** (`memory/material_zone_layer_linear_edges_cost_2026-08-13.md`). ~4 sessions on top of MUV; ~7 sessions combined (some PdfOverlayEditor toil is shared, so it's not a naive 4+4). RECOMMENDATION: ride with MUV — the two-key legend + linear edges + honesty layer is the second half of ONE instrument, not a separate feature. Fits September with margin. Awaits ruling.
- BACKLOG.md rewritten to match Howard's ORDER verbatim from the pro-quotes reply: 1. Ledger truncation ✓ · 2. P0 chips + JobInfoPanel · 3. Material Zone Layer MUV (APPROVED as scoped, ~4 sessions) · 4. Verdict-and-Triage card · 5. Ledger UI on Blueprint card · 6. EST-040221 orientation flip · 7. Reconciliation (bar c) HELD.
- MUV APPROVED as scoped (Howard 2026-08-13 pro-quotes section 3): original elevation PDF pages as canvas, user-adjustable polygons, 3/16" = 1'-0", `qty_src == "human"` survives rebuilds, simplified sheets kept as secondary, excludes snapping / PDF export / multi-user. Ready to build on the next ruling.
- PURITY STILL HELD (do not prompt harder): gable over-count 6 vs 4, porch 195 vs 99, corners 12/8 vs 8/4. Nothing applies to EST-886440. Integral-J stays ON.

## SESSION 2026-08-13 (part 90) — SEND-11 PRO-QUOTES REPLY 2: FIVE NEW SKIPS RETIRED · LINEAR-EDGES SCOPED WALK-READY · BACKLOG REORDERED
- FIVE NEW SKIPS DIAGNOSED AND CLOSED. Baseline was 5 skipped; my ledger fix silently raised it to 10. Cause: the session fixture in `test_send11b_protected_ledger_truncation_honest_2026_08_13.py` hardcoded `http://127.0.0.1:8001/api` and password `"Passw0rd!"` — the pod's auth uses the shared `TEST_PASSWORD` and the external ingress `BASE_URL`. The (passing) `test_guard_extension_2026_08_11.py` wires against those shared helpers correctly; my file did not. Fix: one commit switching to `api_base.BASE_URL` + `creds_for_tests.TEST_PASSWORD` and dropping the missing-`token` KeyError (auth returns a cookie; the requests.Session's jar carries auth on subsequent calls). Every one of the five HTTP pins now RUNS. Report: `memory/five_new_skips_diagnosis_2026-08-13.md`.
- HANDBACK CLEAN: `RECORDED: 2026-08-13 08:28 UTC · c8adc41 · CLEAN · RESULT: 2327 passed, 5 skipped, 6 warnings in 149.84s (0:02:29) · INGRESS SMOKE: 4 passed`. Passed up by 5. Skips back to baseline. Howard's "either it runs or it does not count" rule closes this loop.
- LINEAR-EDGES SCOPED WALK-READY per Howard's "scope the increment NOW so it can begin the day I finish walking, and do not let the gap fill with other work." Four sessions, one file per session: `memory/linear_edges_walk_ready_scope_2026-08-13.md`. Sessions: (1) edge model + math + rebuild survival + tests; (2) polyline tool + tool-switch + LF readout; (3) LegendPanel two-key layout + click-to-highlight; (4) honesty layer (`render_status` + `HonestyOverlay` + regression walk). Extended 5-point walk bar (6/7/8) added.
- BACKLOG.md rewritten to Howard's pro-quotes-reply-2 order verbatim: **1. P0 chips + JobInfoPanel · 2. Material Zone Layer MUV (then Howard walks it) · 3. Linear edges + legend + honesty (starts day walk ends) · 4. Envelope[T] prototype on ledger endpoint (after MUV) · 5. Verdict-and-Triage card · 6. Ledger UI on Blueprint card · 7. EST-040221 orientation flip · 8. Reconciliation (bar c) HELD.** Overrides recorded so the same demotion never happens twice.
- MUV WALK BAR (Howard's words, preserved verbatim in BACKLOG): "I open a real elevation page. I draw or drag a polygon over a wall. The square footage changes. The material line changes with it. It is marked as MY entry, not the app's. It is still there after a rebuild." If all five true → increment starts.
- ENVELOPE[T] REDUCED FROM ~2.5-SESSION REFACTOR TO PROTOTYPE ONLY (Howard: "do not commit 2.5 sessions on my word ... take your own recommendation: PROTOTYPE Envelope[T] on the ledger endpoint we just fixed, verify the contract holds against a real path, and REPORT. Then I rule on the rest."). Comes AFTER MUV. Moves from position 4 in the earlier queue to position 4 in this queue — same slot, different meaning.
- PURITY STILL HELD (do not prompt harder): gable over-count 6 vs 4, porch 195 vs 99, corners 12/8 vs 8/4. Nothing applies to EST-886440. Integral-J stays ON.

## SESSION 2026-08-13 (part 91) — SEND-11 PRO-QUOTES REPLY 3: FIVE P0 CHIPS LANDED · LINEAR-EDGES S3 GAINS STATUS · BASELINE-5 NAMED
- FIVE P0 CHIPS LANDED (position 1 half-1). Every file named in the baseline burn-down report §1 P0 batch now renders a `SurfaceAccessChip` (state + wayOut) in the `return null` branch that used to hide the surface: SidingProfileChip (no carrier), PhotoFillinGateBanner (mid-fetch loading), OpeningsReviewCard (review computing), CompositionTrace (empty smart-engine composition), FinalJobSurface (gates loading). Baseline null-return census updated (5 entries struck with the pro-quotes-reply-3 comment; the future-facing pin keeps new chip branches honest).
- LINEAR-EDGES SCOPE AMENDED per Howard's ruling: "render_status must be part of S3. The legend must never show bare numbers, even for one session." Document updated (`memory/linear_edges_walk_ready_scope_2026-08-13.md`) — S3 now ships status badges beside every legend quantity (`✓ verified` / `⚠ unverified` / `✎ your entry` / `— unread`); S4 is on-drawing hatching only; new S3 pin `test_legend_status_never_lies`.
- BASELINE FIVE SKIPS NAMED (low priority per Howard). Report at `memory/baseline_five_skips_named_2026-08-13.md`. Classified: one tombstone (iter-6 obsolete per-company material override test — `test_estimator_api.py:147 TestCatalogPerCompany.test_catalog_isolated_between_companies`); four cadence-gated ingress smoke tests (`test_external_smoke_2026_08_07.py:42/50/59/90`, gated by `TEST_API_EXTERNAL=1`, printed on the handback stamp as `INGRESS SMOKE: 4 passed`). Neither class is flaky. Recommendations report-only: invert the tombstone into a positive "the current shape refuses this" assertion; add a skip-reason-class lint pin. Awaits ruling.
- THREE MISFILE MOVES SCOPED WALK-READY (position 1 half-2, `memory/three_misfiled_surfaces_move_scope_2026-08-13.md`). Move A (Run history chip, ~45 min); Move B (Profile picker relocation, ~2 h); Move C (Accent injection relocation, ~2 h). ~5 h combined ≈ one focused session. Next turn starts inside a known plan.
- HANDBACK CLEAN: `RECORDED: 2026-08-13 08:54 UTC · bdf1a80 · CLEAN · RESULT: 2327 passed, 5 skipped, 6 warnings in 147.38s (0:02:27) · INGRESS SMOKE: 4 passed in 1.56s`. Same 2327 passed / 5 skipped baseline held through the chip-land session — the census amendment kept discipline.
- QUEUE (Howard 2026-08-13 pro-quotes reply 3, unchanged in this session):
  1. P0 CHIPS ✓ half-landed (5 chips ✓; 3 moves walk-ready) · 2. Material Zone Layer MUV · 3. Linear edges + legend + honesty (S3 gained status) · 4. Envelope[T] prototype (after MUV) · 5. Verdict-and-Triage card · 6. Ledger UI · 7. EST-040221 orientation flip · 8. Reconciliation HELD.
- PURITY STILL HELD (do not prompt harder): gable over-count 6 vs 4, porch 195 vs 99, corners 12/8 vs 8/4. Nothing applies to EST-886440. Integral-J stays ON.

## SESSION 2026-08-13 (part 92) — SEND-11 PRO-QUOTES REPLY 4: POSITION 1 COMPLETE — TOMBSTONE INVERTED + SKIP-LINT PIN + THREE MISFILE MOVES DONE
- POSITION 1 COMPLETE. All four sub-items shipped in this one session.
- TOMBSTONE INVERTED. `test_estimator_api.py:147 TestCatalogPerCompany` becomes `test_material_overrides_are_structurally_impossible` — a positive assertion of the current shape. Three assertions carry the ruling: (1) `CatalogOverridesIn` body model rejects the old `sections: [...]` shape (pydantic 422), (2) a `mat` value inside a valid override is silently stripped by `routes/catalog.py:151`, (3) company isolation via `company_id` holds under a real PUT. A test that asserts what is true now guards something; a skipped tombstone teaches nothing.
- SKIP-REASON-CLASS LINT PIN. `tests/test_skip_reason_class_registry_2026_08_13.py` — registry names eleven legal classes (tombstone, cadence:tape_check, cadence:external, env:mongo, env:live_auth, env:llm_key, env:signup_code, env:fixture_estimate, env:fixture_data, env:fixture_ledger, env:backend_url); AST-walking census refuses any `pytest.skip` reason not beginning with a registered tag. Constant reasons AND the leading literal portion of f-strings are classified. Bulk-annotated 20+ pre-existing anonymous skips in the same commit — the build fails on the next anonymous skip. That is the registry-plus-census pattern applied to skips, the same move that closed seams, TTL indexes, schema consumers, surfaces, and normalizer fields on first run.
- MOVE A LANDED. `RunComparisonChip.jsx` — new estimate-scope chip on JobInfoPanel's Blueprint tile. Reads `/api/measure/ai-measure/history/{eid}`, renders `"N completed reads · model1, model2..."` (or `"No completed reads yet"`, or `"loading"`) — never invisible.
- MOVES B AND C already accomplished 2026-08-09 (per the ruling comments at `EstimateEditor.jsx:479` for `SidingProfileChip` and `:485` for `PerElevationBreakdownCard` — both mount at estimate scope with their own dialogs). JobInfoPanel chip copy for both surfaces updated to name the true way out (the vinyl tab of the estimate page) instead of the misleading "open the run dialog" text.
- HANDBACK CLEAN: `RECORDED: 2026-08-13 09:22 UTC · 18ecb73 · CLEAN · RESULT: 2331 passed, 4 skipped, 6 warnings in 152.00s (0:02:31) · INGRESS SMOKE: 4 passed in 1.29s`. Passed rose by 4 (2327 → 2331) — three from the new skip-lint pin, one from the tombstone inversion running instead of skipping. Skipped fell by one (5 → 4) as the tombstone left the skip class.
- QUEUE (Howard 2026-08-13 pro-quotes reply 4, unchanged): 1. Position 1 COMPLETE ✓ · **2. MATERIAL ZONE LAYER — MUV (up next)** · 3. Linear edges + legend + honesty (S3 with status) · 4. Envelope[T] prototype on ledger endpoint · 5. Verdict-and-Triage card · 6. Ledger UI on Blueprint card · 7. EST-040221 orientation flip · 8. Reconciliation HELD.
- MUV WALK BAR (Howard's words, restated verbatim from pro-quotes reply 4): "I open a real elevation page. I draw or drag a polygon over a wall. The square footage changes. The material line changes with it. It is marked as MY entry, not the app's. It is still there after a rebuild." Five things. If all five true → MUV passes → linear-edges increment starts the same day.
- PURITY STILL HELD (do not prompt harder): gable over-count 6 vs 4, porch 195 vs 99, corners 12/8 vs 8/4. Nothing applies to EST-886440. Integral-J stays ON.

## SESSION 2026-08-13 (part 93) — MUV SESSION 1: BACKEND + POLYGON-WRITES-ARE-HUMAN-ENTRY + CHIP-DESTINATION-VERIFIED PIN
- TWO PRE-CONDITIONS LOCKED IN BEFORE THE BUILD (both ruled 2026-08-13 pro-quotes reply 5):
  1. **Polygon writes are HUMAN ENTRY on protected estimates.** `untouchable.ledger_human_write` docstring now names `pdf_overlay_polygon` alongside tape_check, profile_annotations, flag_checklist, tape_check_score. Every PUT/DELETE on the pdf-overlay endpoint rides above the untouchable freeze AND lands in `protected_estimate_ledger`. Built in at MUV BIRTH, not discovered on the walk.
  2. **Chip-destination-verified pin.** `tests/test_chip_destination_verified_2026_08_13.py` scans every `SurfaceAccessChip` wayOut in the frontend and verifies the named destination exists in the codebase. Verifier map covers Vinyl tab / blueprint read dialog / HOVER upload modal / photo read; unverified wayouts capped at 12 (baseline 8 + 3 slack). "A chip that names the wrong way out is worse than no chip."
- MUV SESSION 1 BACKEND LANDED. `routes/pdf_overlay.py` exposes GET/PUT/DELETE `/api/estimates/{eid}/pdf-overlay`. Deterministic polygon→sqft via shoelace at 3/16"=1'-0" scale using each sheet's rendered inch dims (defaults 8.5"×11" portrait). `apply_overlay_to_takeoff` sums polygon sqft per `(material_class, face_id)` and stamps `qty_src="human"` on matched lines — rebuild survival delegated to hover.py's existing `qty_src=='human'` shield.
- MATERIAL CLASSES (MUV): `{siding, soffit, accent, trim}`. FACE IDS (MUV): `{front, back, left, right}` + `dormer:<label>`. Additions are one-line entries.
- SEAM REGISTERED: `pdf_overlay_polygon_write` names the write class + human-entry rule + rebuild-survival delegation. W-REGISTER: `pdf_overlay.py` added to `LINE_WRITER_REGISTER` with clobber coverage.
- LIVE-VERIFIED ON EST-886440 (the estimate Howard rules the walk goes on): `PUT overlay: 200 · ledger latest kind: pdf_overlay_polygon (meta names polygon_id/page/face_id/material_class) · DELETE: 200`. The walk is possible.
- TEST PIN: 16 cases in `test_pdf_overlay_muv_session1_2026_08_13.py` across seam registration, polygon math (full-page = 2659 ft², quarter-area, winding order, degenerate), apply_overlay contracts (stamp qty_src=human on matched only, sum multi-polygons, drop unmatched), face + material + vertex validation, and the untouchable docstring carrying pdf_overlay_polygon in the human-entry list.
- HANDBACK CLEAN: `RECORDED: 2026-08-13 11:36 UTC · d1b1aea · CLEAN · RESULT: 2349 passed, 4 skipped, 6 warnings in 147.67s (0:02:27) · INGRESS SMOKE: 4 passed in 1.42s`. Passed rose by 18 (2331 → 2349). Zero regressions.
- REMAINING MUV SESSIONS (per `memory/material_zone_layer_cost_2026-08-13.md` and `memory/linear_edges_walk_ready_scope_2026-08-13.md`): Session 2 — `PdfOverlayEditor.jsx` frontend page (rasterised PDF page canvas, click-to-add polygon, drag-vertex, live sqft readout). Session 3 — line-editor / takeoff-panel bindings and estimate-page mount. Session 4 — testing_agent_v3_fork end-to-end walk against Howard's 5-point bar on EST-886440.
...
## SESSION 2026-08-14 (SEND-18) — RULINGS P/Q/R/S/O: census pin ratchet + the three readers converted to status
- **RULING P + S — CENSUS PIN GREEN, as a scanning ratchet.** `tests/test_no_raw_wall_dim_in_priced_path_2026_08_14_send17.py` rewritten: SCANS all four priced modules (AST), keyed on FILE::FUNCTION::SYMBOL (line numbers dropped), diffs against a 3-class baseline `_raw_wall_dim_baseline.txt`. A NEW raw wall-dim read fails and is named; the ratchet turns ONE WAY (removal = conversion, addition needs a send-ruling — stated at top of the file). Prints `census pin GREEN — N baselined reads, K PENDING_CONVERSION (...)` every run (green ≠ clean). Three classes: PENDING_CONVERSION / OUT_OF_SCOPE_N_BUT_PRICED / AUDITED_SAFE, discriminated by "does this feed a priced quantity."
- **DETECTOR SCANS (Howard's P question answered):** it is a baseline DIFF of an AST scan, so it fails on a genuinely new read, not only a changed known one.
- **RULING Q + R — THE THREE READERS CONVERTED:**
  - *corner/OSC average (lp_package):* `_ai_avg_wall_height_ft` / `avg_wall_height_ft` KILLED for priced corner heights. `_corner_height_ft` now returns a `Quantity` (Ruling R): override→DERIVED; walls agree→DERIVED; walls DISAGREE→MAX + PARTIAL naming both walls+heights (min() retired — it was a silent under-count); ANY height-dead touching wall→NOT DERIVABLE naming the dead wall. OSC/ISC lines carry status; NOT DERIVABLE ⇒ qty None, empty price, `blocks_gate`, flagged.
  - *base-course starter (profile_callouts):* base-course/gable width now routes through `wall_width_for_pricing`; a killed width ⇒ NOT DERIVABLE face, never a silent-0 base course.
  - *batten (lp_smartside_formulas):* `board_batten_batten_pieces_status` makes the `+1 run × wall_height` term explicit — PARTIAL (field panels derivable, run term disclosed-excluded) when the batten wall height is unconfirmed, never a folded silent 0.
- **SYNTHETIC PINS (mandatory, send-18 Q):** `test_ruling_qr_corner_refusal_...send18.py` (11) + `test_ruling_q_basecourse_batten_...send18.py` (6) deliberately kill a height/width and assert NOT DERIVABLE / PARTIAL — the refusal branch is exercised on purpose (fixtures' heights survive since send-13).
- **RULING O — SKIP ROSTER IN EVERY HANDBACK.** `scripts/skip_roster.py` + `handback_green.sh` (now `-rs`): prints every skip, its ruling text, age in sends; skips >3 sends print blocker + who owes the unblock. Handback also echoes the census status line.
- **HELD RULINGS UN-SKIPPED (register discipline):** send-15 Ruling G + Ruling H and send-16 G/H-wiring tests converted from held-skips to real passing pins (they are now built). Remaining held: send-15 Ruling F (frontend elevation-sheet render harness), send-13 elevation-sheet/money-line/drawn-geometry (Howard's plan-set + reconciliation).
- **REPORTS:** (1) `avg_wall_height_ft` NOT deleted — it retains legitimate NON-priced uses (gutter downspout drop + roofline mitre count in hover.py, blueprint-elevation height fallback, AI extraction schema); only lp_package's priced corner fallback was killed. (2) Ruling M fixture manifest EXISTS (`memory/fixture_manifest.json` + `test_bar_b_fixture_manifest`) and showed NO drift after the conversions — Boni line-set unchanged (heights survive, so the refusal branch never fires on it, exactly as Howard predicted). (3) BATTEN is OUT-OF-SCAN: its height is the human checklist scalar `_bb_wall_height_ft` (lp_package_routes → hover.py), not a raw AI dim in the four scanned modules — noted in the baseline header, converted at the formula boundary; awaiting a ruling on whether hover.py joins PRICED_MODULES. (4) FINDING for Howard: with exact-agreement DERIVED, real AI wall heights (e.g. Letrick 8.5 vs 8.3) rarely match to the decimal, so multi-wall corners read PARTIAL broadly (annotation, not a gate block); no tolerance was invented (that would be tuning). (5) OPEN against Ruling R: split-corner (tall wall meets short wing = two corners?) recorded, not built.
- **PURITY:** `quantity.py` registered in LP_CORE_MODULES (pure stdlib) to keep the fork-boundary pin green. EST-886440 PROTECTED, Integral-J ON, no derivation reads another estimate.
- **SUITE:** 2450 passed, 9 skipped, 0 failed (159s) + ingress smoke 4 passed. (Prior: 2349 passed / 4 skipped.)
- **HOWARD'S SIDE (unchanged):** re-fire EST-713272 four-face report (stamps A/B before money is wired); paste sends 8–12 into rulings_archive/; a real non-Boni plan set (unblocks drawn-geometry skip). S3 linear-edges + Law B stay HELD behind the four-face report.


## SESSION 2026-08-14 (SEND-19) — EST-713272 four-face RE-FIRE + Rulings T/U/V/W
- **PRIORITY DELIVERED — EST-713272 four-face report (re-fired from the 5 stored ai_blueprint_runs):**
  - Run 1 (13:14, opus-4-5): ALL FOUR faces DERIVED — front 920, back 1160, left 780 + 311 gable, right 780 + 311 gable = **siding 4,461 ft²**, n_faces_not_derivable=0. Widths were read.
  - Runs 3–5 (15:31 / 17:33 / 18:42, same model): ALL FOUR faces **NOT DERIVABLE** ("wall width not read — area not derivable"), siding = 200 ft² (chimney chase appendage ONLY), 6 faces named not-derivable.
  - **VERDICT:** the derivation (rulings A/B) is HONEST in both cases — it derives when the widths exist and REFUSES + names them when they don't; it never fabricated a width. The material list is honest. The variance is an UPSTREAM AI VISION-READ instability (same sheet + model → 4,461 vs refuse; stability block also shows windows 20↔4, roof planes 3↔4), NOT a takeoff-logic problem. Rulings A/B stamp GREEN on the derivation; the read instability is a separate item for Howard.
- **RULING T — corrected Ruling R (PARTIAL was the error).** Corners never emit PARTIAL now. `_corner_bounds` returns (min,max); `_corner_height_ft` → DERIVED at MAX for a disagreement (never averaged), NOT DERIVABLE only for a dead/absent touching wall. The takeoff asks Ruling T's real question — does the disagreement CHANGE THE STICK COUNT (count@min vs count@max)? SAME count → DERIVED, disagreement on the READ-BACK field only (`readback`), no line note, no gate. DIFFERENT count → MAX + LINE annotation naming both walls, both heights, both counts. Synthetic pins cover both. "Does it change the quantity?" test reach reported: it applies to corner posts (stock lengths); other stock-length material (OSC/ISC sticks) already flows through the same takeoff.
- **PARTIAL audit (Ruling T):** nothing else in the codebase currently emits PARTIAL except `board_batten_batten_pieces_status` (batten +1-run term when wall height unconfirmed) — that IS a true disclosed SUBSET (field panels derivable, run term excluded), so it correctly stays PARTIAL.
- **RULING U — baseline shrink accounted (10→4).** REMOVAL_LOG added to `_raw_wall_dim_baseline.txt` itemising all six removals: 3 re-key merges (duplicate line entries collapsed by the file/function/symbol key) + 3 Ruling-Q conversions. The census pin now prints the removal count every run.
- **RULING W — hover.py joined PRICED_MODULES.** Census `_scan` now handles `async def` too. Batten answer: `_bb_wall_height_ft` is set ONLY from a CLOSED taped-height checklist — NO fallback to any AI dim/default/prior value → classed **AUDITED_SAFE ("human-confirmed, no fallback")**.
- **RULING V — the two hover `_ai_avg_wall_height_ft` reads DO reach material** (downspout coil LF; roofline mitre count). Catalogued as **PENDING_CONVERSION** with the exact silent substitutes named to kill (`_ai_story_count or 1`; hardcoded 9'/h=9.0). NOT half-converted this pass — a faithful Q-conversion needs the Quantity/NOT-DERIVABLE wiring through the gutter/downspout/mitre line builders (its own focused pass, like corners), and send-19 said everything yields to the four faces. Reported, not silently dropped.
- **Census now:** GREEN — 8 baselined reads, 2 PENDING_CONVERSION (both hover avg), 6 removals logged. **Suite: 2452 passed, 9 skipped, 0 failed** (167s); no fixture-manifest drift.
- **HOWARD'S SIDE (acceptance gate, still open):** the four-face re-fire above is reported from stored runs, but Howard's own re-fire + confirmation is the stamp; paste sends 8–12 into rulings_archive/; a real non-Boni plan set. EST-886440 PROTECTED, Integral-J ON, purity pin holds.
