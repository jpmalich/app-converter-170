...
- **Iter 19** — **English/Spanish i18n** (Feb 2026): full bilingual support with EN/ES toggle. **Infrastructure**: new `/app/frontend/src/lib/i18n.jsx` (LangProvider + `useT()` hook, localStorage-persisted `ui-lang-v1` key, auto-detect browser preference on first load) + `/app/frontend/src/lib/dictionaries.js` (~150 keys across nav, auth, dashboard, catalog, estimate editor, quote modal, accept page, email/PDF body). New `LangToggle` pill component (EN/ES) appears in the authed header AND on Login (top-right) AND on the public Accept page. **Coverage**: Layout/nav, Login (welcome heading + role labels + invite copy + access-code helper), Dashboard (filter chips · stats sublabels · empty state · badges · toasts), Catalog (page title · tier badge · column headers · save/reset confirms), full EstimateEditor (Job Info · Waste/Tax/Profit · Margin/Markup buttons · Sticky bar · Summary stats · all line-item headers · misc rows · loading state), QuoteModal (subject · personal note + auto-detected greeting · "Send in: EN/ES" picker · email/PDF buttons), AcceptPage (heading · note · checkbox consent · disclaimer · success state — auto-detects `?lang=es` from URL). **Per-estimate language picker** in QuoteModal lets the same contractor send English to one customer and Spanish to another — sets the email HTML language, the auto-greeting, the subject line, and appends `?lang=es` to the accept link so the customer's hosted page matches. Greeting auto-updates when the contractor flips the picker BUT respects custom-typed messages (the ref-based diff doesn't clobber edits). PDF inherits the language automatically because it renders from the same frontend-built HTML. Zero backend changes — pure frontend feature. Verified end-to-end: EN/ES toggle works on Login, Dashboard, EstimateEditor, QuoteModal · custom messages preserved · `?lang=es` deep link flips Accept page · 0 page errors. ESLint clean.
