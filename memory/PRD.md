...
2. *Type conflicts flagged, never resolved:* same dedupe location with OSC/ISC disagreement → `corner_type_conflict`, both recorded, amber. Dedupe key = wall-pair + position tolerance along the wall — **tolerance defined here: two sightings dedupe to one corner when they share the wall-pair (or single wall for appendage corners) AND their positions along the wall agree within ±10% of that wall's length or ±2 ft, whichever is larger** (so two sightings of one chase corner can't split into two corners).
