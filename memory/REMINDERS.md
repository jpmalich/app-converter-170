...
- **OPEN AFTER RERUN 840b34e8 — RESOLVED**: Howard RULED "the COUNT column governs counts" (2026-08-09) and it is ENFORCED AT THE SEAM (`_enforce_count_column`): qty rewritten from printed cells, no-cell marks contribute nothing, proven on rerun a4cbce91 (16 by the column: A 9=2+7 · B 1 · C 5 · D 1).
...
- **SILENT-TRUNCATION CLASS — BUILT (2026-08-09)**: seam_accounting.py registry + ledger + AST detector; an unregistered removing seam raises; the detector was WIDENED (send 7) to slices and min(len()) caps after the register audit proved comprehension-only scanning missed the page cap (same evasion class as D5). NAMED LIMIT: assigning len() to a variable before min() still evades the AST scan — the page cap itself is accounted + pinned by test instead.
- **ROOF SECOND PASS — REGISTERED + RULED (2026-08-09 send 7)**: the garage-triggered second read's overwrites are ledgered old→new (`roof_pass_overwrite`); NEVER-TOUCH RULE enforced — an EVIDENCED value is never replaced by an unevidenced one (refusals named on the rail); roof-pass schema now demands DIM corner heights. See memory/roof_pass_mechanism_2026_08_09.md.
- **CORNER AGREEMENT-OR-FLAG (Howard ruled 2026-08-10)**: max-wins acceptance is DEAD. A disagreeing corner walk KEEPS THE PRIMARY and fires a loud `corner_walk_conflict` PRINTING BOTH NUMBERS ("primary read {p} outside/{q} inside, roof pass {r}/{s}") — Howard counted NINE on his own markup; two numbers show at a glance when neither is right. Fill still allowed when the primary carried no walk. Heights ride only an AGREED or FILLED walk, under never-touch.
- **RAKES — THE PLANE SUM GOVERNS (Howard ruled 2026-08-10)**: whenever planes carry rake figures, the plane sum IS the rake number (exact mirror of the eaves rule; `_rakes_plane_summed`). Larger-wins died here too — a bare top-level number never beats evidenced planes.
- **EAVE/RAKE ORIENTATION CHECK (built 2026-08-10, the EST-040221 instrument)**: on a simple gable roof, eaves and rakes sit on OPPOSITE wall pairs, always. An eave figure matching the GABLE pair better than the eave pair (pairs ≥15% apart) flags loud `eave_rake_orientation` with all three numbers. The flip itself remains unattributed — primary read and plane-append are the candidates.
- **MAX-WINS PATTERN SEARCH (2026-08-10, report delivered, NOTHING CHANGED per order)**: full list in memory/max_wins_census_2026_08_10.md. True larger-wins races found: corners + rakes (both killed by ruling). Remaining named: box-model rect uses max of opposing wall widths (can SUPPRESS wing_flag), porch wall-side picks the longer dim as wall-abutting (orientation assumption), gable ridge cross-check compares per-elevation MAX rise (comparison instrument, inert). Everything else = ruled FLOORS (min-2 end caps, min-4 J blocks, etc.) or geometry box math — not the class.
- **PAGE CAP SAYS SO (ruled 2026-08-09 send 7)**: dropped plan-set pages are a registered seam (`pages_truncated`), ledgered and flagged LOUD on the card ("N pages in the set → M read"). Defaults raised 12→20 (hard 20→28) with a 24 MB total-size recompress guard (percent loc boxes are scale-invariant).
- **CONSUMER KEY CLASS WIDENED (ruled 2026-08-09 send 7)**: every model-schema consumer read by exact key (test_schema_consumer_keys); founding instance `w.get("accents")` vs `accent_profiles` FIXED (the callout census's accent leg was dead — false callout_omitted on accent-only families); the detector's FIRST RUN caught instance #2: vent_count/shutter_count consumed but never in the blueprint schema (Q7 said "wired") — schema now asks for both.
- **NAMED INSTRUMENT LIMITS (registered 2026-08-09 send 7)**: (a) omission census tolerates one-glyph OCR drift of a READ row (`_del1`) — a real sibling one glyph away can be eaten; the mark-merge cross-reference partially compensates. (b) callout census keywords = SHAKE/BATTEN/DUTCHLAP/SCALLOP only — narrower than the mapper's families (no VERTICAL, no NICKEL GAP). (c) the roof second pass reads ≤5 sheets.
- **CALLOUT FABRICATION GAP (named 2026-08-09, NOT ordered)**: the census catches a printed family the read MISSED; nothing catches a family the read INVENTED — a fabricated "SHAKE" quote still creates accent lines. Same fabrication-vs-omission asymmetry closed on schedules, open on callouts. Sits beside the GEOMETRIC OMISSION GAP.
- **EST-886440 GUARD BUILT (2026-08-09 send 7)**: server refuses every write (423) — PUT/PATCH/DELETE/protection-flip/model3d/rederive/hover-lp-run/lp-apply/materialize/order-release/default-profile. Reads, reruns, and duplication stay open. backend/untouchable.py.
