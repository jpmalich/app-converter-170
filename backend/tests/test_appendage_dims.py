...
EST_ID = "8f95c9c2-add9-416a-92f3-786a4ea2ce83"  # letrick 7-14-26 7pm (photo) — READ-ONLY clone source (SEND-107)
BP_EST_ID = "db82ec7a-3177-406d-a602-927255e9e10e"  # has blueprint runs — READ-ONLY clone source (SEND-107)
KEY = "appendage:right"
...
# SEND-107: writes moved off the REAL estimates onto disposable clones
# (runs copied estimate-scoped so preview/offers resolve). The old
# "leave estimate as found" teardown is retired — clones are deleted.
@pytest.fixture(scope="module")
def est_id(session):
...
@pytest.fixture(scope="module")
def bp_est_id(session):
    cid = clone_estimate(session, API, BP_EST_ID, copy_runs=True)
    yield cid
    drop_clone(session, API, cid)
...
def test_chase_height_rederives_osc_sticks(session, est_id):
    base = _osc_line(_preview(session, est_id))
    base_qty = base["qty"]
    ...
    after = _osc_line(_preview(session, est_id))
...
def test_absurd_height_flags_on_preview(session, est_id):
    r = session.post(f"{API}/estimates/{est_id}/lp-appendage-dims",
                     json={"key": KEY, "field": "height_ft", "value": 60}, timeout=15)
    assert r.status_code == 200
    try:
        flags = _preview(session, est_id)["appendage_dim_flags"]
...
def test_validation_rejects_bad_input(session, est_id):
...
def test_blueprint_offers_surface_on_bp_estimate(session, bp_est_id):
...
def test_photo_only_estimate_offers_nothing(session, est_id):
    r = session.get(f"{API}/estimates/{est_id}/lp-appendage-dims", timeout=15)
    assert r.status_code == 200 and r.json()["offers"] == []


def test_accept_from_blueprint_tags_confirmed(session, bp_est_id):
